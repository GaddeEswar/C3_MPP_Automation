...
class OfflineValidation():
    def __init__(self,TestID='',ProjectJson='',TracePath='',mode='TPR',product="C3"):
        print('val call')
        self.TestID =TestID
            ...
            if len(self.Json_TC['other_checks_details'][str(index)])>0:
                for apchk in self.Json_TC['other_checks_details'][str(index)]:
                    ...
                    if apchk in ['PowerMatch']:
                        ...
                        GetQIConfiguration = APIOperations(url=self.JapiData[self.product][self.mode]['GetQIConfiguration'],retype='json')
