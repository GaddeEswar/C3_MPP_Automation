...
class CTSChecks_MPP_TPR1():
    ...
    def Cloak_Compatibility(self,Flow_limit,Check):
        ...
        if len(sadc_open1)>2:
            res.append([f"SADC Open Stream packet found at {round(sadc_open1[0],3)} sec", "Pass"])
            ...
            if sadc1_resp is not None and self.file_list[sadc1_resp]['pktType'] == "SDSR" and "ACK" in self.file_list[sadc1_resp]['value']:
                res.append([f"SDSR/ACK response found at {round(self.file_list[sadc1_resp]['startTime'],3)} sec, Expected: SDSR/ACK", "Pass"])
            else:
                SDSR_ACK_flag = False
                res.append([f"{self.file_list[sadc1_resp]['pktType']}_{self.file_list[sadc1_resp]['value']} found at {round(self.file_list[sadc1_resp]['startTime'],3)} sec, Expected: SDSR/ACK", "Fail"])
            ...
            if len(sadt1)>2:
                ...
                if cert_type1 == "Get_Certificate":
                    res.append([f"{cert_type1} found in SADT found at {round(sadt1[0],3)} sec, Expected: Get_Certificate", "Pass"])
                else:
                    res.append([f"Invalid {cert_type1} found in SADT found at {round(sadt1[0],3)} sec, Expected: Get_Certificate", "Fail"])
                ...
                if len(sadc_close1)>2:
                    res.append([f"SADC Close Stream packet found at {round(sadc_close1[0],3)} sec", "Pass"])

                    atn1 = self.PktMethod.GetPacketDetails(packet='ATN',limit=[sadc_close1[2], Flow_limit[1]],Type="Response")
                    if len(atn1)>2:
                        res.append([f"PTx sent ATN after Get_Certificate at {round(atn1[0],3)} sec", "Pass"])

                        dsr1 = self.PktMethod.GetPacketDetails(packet="DSR",value="POLL",limit=[atn1[2], Flow_limit[1]],Type="Packet")
                        if len(dsr1)>2:
                            res.append([f"TPR sent DSR(POLL) at {round(dsr1[0],3)} sec", "Pass"])
                            ...
                            if len(sadc_open2)>2:
                                res.append([f"SADC Open Stream(Response) found at {round(sadc_open2[0],3)} sec", "Pass"])
                                idx = sadc_open2[2]
                                sadtx_cnt = 0
                                    ...
                                    if len(sadtx)>2:
                                        ...
                                        if sadtx_cnt == 1:
                                            ...
                                            if cert_type2 == "Certificate":
                                                res.append([f"{self.file_list[sadtx[2]]['pktType']} response with {cert_type2} found at {round(sadtx[0],3)} sec, Expected: Certificate", "Pass"])
                                            else:
                                                res.append([f"{self.file_list[sadtx[2]]['pktType']} response with {cert_type2} found at {round(sadtx[0],3)} sec, Expected: Certificate", "Fail"])
                                        else:
                                            res.append([f"{self.file_list[sadtx[2]]['pktType']} response found at {round(sadtx[0],3)} sec", "Pass"])
                                        idx = sadtx[2]
                                    idx += 1
                                ...
                                if len(cert_data1) > 2:
                                    ...
                                    res.append([f"s1 is {s1}","Pass"])
                                    # print("s1:", s1)
                                else: res.append([f"CERTIFICATE is not found", "Fail"])
                                ...
                                if len(clk1)>2:
                                    ...
                                    res.append([f"Cloak initiated with {clk_rsn1} reason at {round(clk1[0],3)} sec", "Pass"])
                                    ...
                                    if len(clk_exit)>2:
                                        ...
                                        if len(atn1)>2:
                                            res.append([f"PTx sent ATN in cloak phase at {round(atn1[0],3)} sec", "Pass"])
                                            ...
                                            else: res.append([f"Cloak Packet not found after ATN", "Fail"])
                                        ...
                                        while id < clk_exit[2]:
                                            ...
                                            if len(clkx)>2:
                                                ...
                                                id = clkx[2]
                                                clk_cnt += 1
                                                res.append([f"Cloak_{clk_cnt} found with {clk_rsnx} reason at {round(clkx[0],3)} sec", "Pass"])
                                                ...
                                                if clk_cnt == 2:
                                                    ...
                                                    if len(report_pkt)>2:
                                                        ...
                                                        if report_rsn == "PRx Identification":
                                                            res.append([f"Report Packet found with PRx Identification at {round(report_pkt[0],3)} sec", "Pass"])
                                                        else: res.append([f"Report Packet found with {report_rsn} at {round(report_pkt[0],3)} sec", "Fail"])
                                                        ...
                                                        if len(PTX_ID_pkt)>2:
                                                            res.append([f"Get Request (PTx Extended Identification) packet found at {round(PTX_ID_pkt[0],3)} sec", "Pass"])
                                                            ...
                                                            if respid2 is not None and self.file_list[respid2]['pktType'] == "Extended Power Transmitter Identification":
                                                                res.append([f"Extended Power Transmitter Identification response found at {round(self.file_list[respid2]['startTime'],3)} sec", "Pass"])
                                                            else:
                                                                res.append([f"Extended Power Transmitter Identification response not found", "Fail"])
                                                        else:
                                                            res.append([f"Get Request (PTx Extended Identification) packet not found", "Fail"])
                                                

                                                    else: res.append([f"Report Packet not found", "Fail"])
                                                    break
                                            else: res.append([f"Cloak Packet_{clk_cnt+1} not found", "Fail"])
                                        ...
                                        if len(atn2)>2:
                                            res.append([f"PTx sent ATN after Cloak exit at {round(atn2[0],3)} sec", "Pass"])

                                            dsr2 = self.PktMethod.GetPacketDetails(packet="DSR",value="POLL",limit=[atn2[2], end],Type="Packet")
                                            if len(dsr2)>2:
                                                res.append([f"TPR sent DSR(POLL) at {round(dsr2[0],3)} sec", "Pass"])

                                                # Certificate 2
                                                sadc_open3 = self.PktMethod.GetPacketDetails(packet='SADC',value="Open Stream:",limit=[dsr2[2], end],Type="Response")
                                                if len(sadc_open3)>2:
                                                    res.append([f"SADC Open Stream Response found at {round(sadc_open3[0],3)} sec", "Pass"])
                                                    ...
                                                    while idx2 < end:
                                                        ...
                                                        if len(sadtx2)>2:
                                                            ...
                                                            if sadtx2_cnt == 1:
                                                                ...
                                                                if cert_type2 == "Certificate":
                                                                    res.append([f"{self.file_list[sadtx2[2]]['pktType']} response with {cert_type2} found at {round(sadtx2[0],3)} sec, Expected: Certificate", "Pass"])
                                                                else:
                                                                    res.append([f"{self.file_list[sadtx2[2]]['pktType']} response with {cert_type2} found at {round(sadtx2[0],3)} sec, Expected: Certificate", "Fail"])
                                                            else:
                                                                res.append([f"{self.file_list[sadtx2[2]]['pktType']} response found at {round(sadtx2[0],3)} sec", "Pass"])
                                                    ...
                                                    if len(sadc_close3)>2:
                                                        res.append([f"SADC Close Stream Response found at {round(sadc_close3[0],3)} sec", "Pass"])
                                                    else:
                                                        res.append([f"SADC Close Stream Response not found", "Fail"])
                                                    ...
                                                    if len(cert_data2) > 2:
                                                        if s2_bytes_length == 17:
                                                            res.append([f"s2 consists of {s2_bytes_length} bytes, Expected: 17 bytes","Pass"])
                                                        else:
                                                            res.append([f"s2 consists of {s2_bytes_length} bytes, Expected: 17 bytes","Fail"])

                                                        s2 = self.Auth_file_list[cert_data2[2]]['header_Payload']['sFieldType'].split(":")[-1].strip()
                                                        res.append([f"s2 is {s2}","Pass"])
                                                        s2_data = s2.split()
                                                        if s2_data[0] == "0x12":
                                                            res.append([f"The first byte of s2 is 0x12, Expected: 0x12", "Pass"])
                                                        else:
                                                            res.append([f"The first byte of s2 is {s2_data[0]}, Expected: 0x12", "Fail"])
                                                        # print("s2:", s2)
                                                        if s1 in s2:
                                                            res.append([f"The bytes in s1: {s1} are equal to the corresponding bytes in s2: {s2}","Pass"])
                                                        else:
                                                            res.append([f"The bytes in s1: {s1} are not equal to the corresponding bytes in s2: {s2}","Fail"])
                                                    else: res.append([f"CERTIFICATE is not reinitiated", "Fail"])
                                                else: res.append([f"SADC Open Stream Response not found", "Fail"])
                                            else: res.append([f"DSR(POLL) not found", "Fail"])
                                        else: res.append([f"ATN not found", "Fail"])
                                    else: res.append([f"MPP_Cloak_Exit not found", "Fail"])
                                else: res.append([f"Cloak ping is not initiated.", "Fail"])
                            else: res.append([f"SADC Open Stream Response not found", "Fail"])   
                        else: res.append([f"PTx didn't sent DSR(POLL) after Get_Certificate at {round(atn1[0],3)} sec", "Pass"])
                    else: res.append([f"ATN not found", "Fail"])
                else: res.append([f"SADC Close Stream Packet not found", "Fail"])
            else: res.append([f"SADT Packet not found", "Fail"])
        else: res.append([f"SADC Open Stream Packet not found", "Fail"])   
            ...
            if len(sadt1)>2:
                ...
                if sadt_respid is not None:
                    if self.file_list[sadt_respid]['pktType'] == "ACK":
                        res.append([f"ACK response received for SADT packet at {round(self.file_list[sadt_respid]['startTime'],3)} sec, Expected: ACK", "Pass"])
                        ...
                        if len(clk1)>2:
                            ...
                            if len(clk_exit)>2:
                                ...
                                if len(sadc_open2)>2:
                                    res.append([f"SADC Open Stream packet found at {round(sadc_open2[0],3)} sec", "Pass"])
                                    ...
                                    if sadc2_resp is not None:
                                        if self.file_list[sadc2_resp]['pktType'] == "SDSR" and "ACK" in self.file_list[sadc2_resp]['value']:
                                            res.append([f"stream_response_1 is SDSR/ACK found at {round(self.file_list[sadc2_resp]['startTime'],3)} sec, Expected: stream_response_1: SDSR/ACK", "Pass"])
                                        else:
                                        
                                            res.append([f"{self.file_list[sadc2_resp]['pktType']}_{self.file_list[sadc2_resp]['value']} found at {round(self.file_list[sadc2_resp]['startTime'],3)} sec, Expected: stream_response_1: SDSR/ACK", "Fail"])
                                    else:
                                        res.append([f"Response not found for SADC/open", "Fail"])
                                    ...
                                    if len(sadt2)>2:  
                                        ...
                                        if cert_type2 == "Get_Certificate":
                                            res.append([f"{cert_type2} found in SADT found at {round(sadt2[0],3)} sec, Expected: Get_Certificate", "Pass"])
                                        else:
                                            res.append([f"Invalid {cert_type2} found in SADT found at {round(sadt2[0],3)} sec, Expected: Get_Certificate", "Fail"])
                                        ...
                                        if sadt2_respid is not None:
                                            if self.file_list[sadt2_respid]['pktType'] == "ACK":
                                                res.append([f"stream_response_2: ACK response received for SADT packet at {round(self.file_list[sadt2_respid]['startTime'],3)} sec, Expected: stream_response_2: ACK", "Pass"])
                                            else:
                                                res.append([f"stream_response_2:{self.file_list[sadt2_respid]['pktType']} response received for SADT packet at {round(self.file_list[sadt2_respid]['startTime'],3)} sec, Expected: stream_response_2: ACK", "Fail"])
                                        else:
                                            res.append([f"Response not found for SADT", "Fail"])
                                    else: res.append([f"SADT not found", "Fail"])
                                    ...
                                    if len(sadc_close2)>2:
                                        res.append([f"SADC Close Stream Packet found at {round(sadc_close2[0],3)} sec", "Pass"])
                                        ...
                                        if sadc_close2_resp is not None:
                                            if self.file_list[sadc_close2_resp]['pktType'] == "SDSR" and "ACK" in self.file_list[sadc_close2_resp]['value']:
                                                res.append([f"stream_response_3 is SDSR/ACK found at {round(self.file_list[sadc_close2_resp]['startTime'],3)} sec, Expected: stream_response_3: SDSR/ACK", "Pass"])
                                            else:
                                                res.append([f"stream_response_3 is {self.file_list[sadc_close2_resp]['pktType']}_{self.file_list[sadc_close2_resp]['value']} found at {round(self.file_list[sadc_close2_resp]['startTime'],3)} sec, Expected: stream_response_3: SDSR/ACK", "Fail"])
                                        else:
                                            res.append([f"Response not found for SADC/close", "Fail"])
                                                ...
                                                if len(sadc_open3)>2:
                                                    ...
                                                    if len(cert_data2) > 2:
                                                        if s1_bytes_length == 17:
                                                            res.append([f"s1 consists of {s1_bytes_length} bytes, Expected: 17 bytes","Pass"])
                                                        else:
                                                            res.append([f"s1 consists of {s1_bytes_length} bytes, Expected: 17 bytes","Fail"])

                                                        s1 = self.Auth_file_list[cert_data2[2]]['header_Payload']['sFieldType'].split(":")[-1].strip()
                                                        res.append([f"s1 is {s1}","Pass"])
                                                        s1_data = s1.split()
                                                        if s1_data[0] == "0x12":
                                                            res.append([f"The first byte of s1 is 0x12, Expected: 0x12", "Pass"])
                                                        else:
                                                            res.append([f"The first byte of s1 is {s1_data[0]}, Expected: 0x12", "Fail"])
                                                        
                                                    else: res.append([f"CERTIFICATE is not initiated", "Fail"])
                                ...
                                else: res.append([f"SADC Open Stream Packet not found for Get_Certificate", "Fail"])
                            else: res.append([f"Cloak Exit packet not found", "Fail"])
                        else: res.append([f"Cloak is not initiated", "Fail"])
                    else:res.append([f"{self.file_list[sadt_respid]['pktType']} response received for SADT packet at {round(self.file_list[sadt_respid]['startTime'],3)} sec, Expected: ACK", "Fail"])
                else:res.append([f"No response received for SADT packet, Expected: ACK", "Fail"])
            else: res.append([f"SADT Packet not found", "Fail"])
        else: res.append([f"SADC Open Stream Packet not found", "Fail"])
    ...
    def Digest_returned(self,Flow_limit,Check):
        ...
        while id < auth_limit[1]:
            ...
            if len(get_dig1) > 2:
                cnt += 1
                res.append([f"GET_DIGESTS_{cnt} packet found at {self.PktMethod.Timeconvert(get_dig1[0])}", "Pass"])
                dig1 = self.CTSMethod.GetAuthPacketDetails(packet="DIGESTS", limit=[get_dig1[2],auth_limit[1]], Type="Response")
                if len(dig1) > 2:
                    res.append([f"DIGESTS_{cnt} response found at {self.PktMethod.Timeconvert(dig1[0])} with following details", "Pass"])
                    ...
                    res.append([f"Authentication_Protocol_Version: {Authentication_Protocol_Version}", "Pass"])
                    res.append([f"Slots_Populated_Mask: {Slots_Populated_Mask}", "Pass"])
                    res.append([f"Slots_Returned_Mask: {Slots_Returned_Mask}", "Pass"])
                    res.append([f"Digests_Returned: {Digests_Returned}", "Pass"])
                    ...
                    id = dig1[2]
                else: res.append([f"DIGESTS_{cnt} response not found.", "Fail"])
            # else: res.append([f"GET_DIGESTS_{cnt} Packet not found.", "Fail"])
            id += 1
        ...
        if len(digests_details) == 5:
            for i,val in enumerate(digests_details[1:]):
                if digests_details[0][0] == val[0] and digests_details[0][1] == val[1] and digests_details[0][2] == val[2] and digests_details[0][3] == val[3]:
                    res.append([f"Digest_1 = Digest_{i+2}", "Pass"])
                else: res.append([f"Digest_1 != Digest_{i+2}", "Fail"])
        else: res.append([f"Only {len(digests_details)} Digests are received instead of 5.", "Fail"])
        ...
        if dig_len == 32:
            res.append([f"Digest_1 = the 32-byte SHA-256 digest of Certificate_1, Expected: 32 bytes", "Pass"])
        else: res.append([f"Digest_1 = the {dig_len}-byte SHA-256 digest of Certificate_1, Expected: 32 bytes", "Fail"])
        return res
    ...
    def Neg_Error_Status(self,Flow_limit,Check):
        ...
        if len(config) > 2:
            res.append([f"Configuration packet found at {round(config[0],3)} sec", "Pass"])
            ...
            if respid is not None:
                ...
                if "MPP" in R1:
                    res.append([f"R1:{R1} response received for configuration packet", "Pass"])
                elif "NAK" in R1:
                    res.append([f"R1:{R1} response received for configuration packet", "Pass"])
                    ...
                    if len(get_error) > 2:
                        res.append([f"Get Request(PTx error status) packet found at {round(get_error[0],3)} sec", "Pass"])
                        ...
                        if respid2 is not None:
                            if self.file_list[respid2]['pktType'] == "PTx Error status":
                                res.append([f"PTx Error status response received at {round(self.file_list[respid2]['startTime'],3)} sec with following details", "Pass"])
                                ...
                                if error_code == 1 and all(ele in error_data.lower() for ele in ["unable to compute","k","est"]):
                                    res.append([f"E1: {error_data}(Value:{error_code}) found in PTx error status response, Expected: Unable to compute KEST (Value:1)", "Pass"])
                                else: res.append([f"E1: {error_data}(Value:{error_code}) found in PTx error status response, Expected: Unable to compute KEST (Value:1)", "Fail"])
                                ...
                                if len(end_pkt) > 2:
                                    res.append([f"{Check["packet1"][0]}({Check["packet1"][1]}) {Check["packet1"][2]} found at {round(end_pkt[0],3)} sec", "Pass"])
                                    ...
                                    if "SRQ" in Check["packet1"][0]:
                                        ...
                                        if respid3 is not None:
                                            ...
                                            if "NAK" in R2:
                                                res.append([f"R2: {R2} response received for {Check["packet1"][0]}({Check["packet1"][1]}) {Check["packet1"][2]}, Expected: NAK", "Pass"])
                                                cvpkp = self.PktMethod.GetPacketDetails(packet="CoilVoltpkpk",limit=[respid3,Flow_limit[1]],Type="TesterMsg")
                                                if len(cvpkp) > 2:
                                                    res.append([f"Uro drops below 200mV at {round(cvpkp[0],3)} sec", "Pass"])
                                                ...
                                                else: res.append([f"Uro drops below 200mV packet not found", "Fail"])
                                            else: res.append([f"R2: {R2} response received for {Check["packet1"][0]}({Check["packet1"][1]}) {Check["packet1"][2]}, Expected: NAK", "Fail"])
                                        else: res.append([f"Response not found for {Check["packet1"][0]}({Check["packet1"][1]}) {Check["packet1"][2]}", "Fail"])
                                ...
                                else: res.append([f"{Check["packet1"][0]}({Check["packet1"][1]}) {Check["packet1"][2]} not found", "Fail"])
                            else: res.append([f"{self.file_list[respid2]['pktType']} response found for Get Request(PTx error status) packet, Expected: PTx Error status", "Fail"])
                        else: res.append([f"Response not found for Get Request(PTx error status) packet", "Fail"])
                    else: res.append([f"Get Request(PTx error status) packet not found", "Fail"])
                else: res.append([f"R1: {R1} response found for Configuration packet, Expected: MPP or ACK or NAK", "Fail"])
            else: res.append([f"Response not found for Configuration packet", "Fail"])
        else: res.append([f"Configuration packet not found", "Fail"])
