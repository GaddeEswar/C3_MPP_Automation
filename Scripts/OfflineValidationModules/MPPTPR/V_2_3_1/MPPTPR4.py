...
class CTSChecks_MPP_TPR4():
    ...
    def PNG_ILL(self,Flow_limit,Check):
        ...
        if len(XID)>2:
                    ...
                    if self.PktMethod.GetPacketType(x) == "Packet" and self.file_list[x]['pktType'] in ["Signal strength","Identification",self.XID_pkt,"Configuration"]:
                        ...
                        if self.file_list[x]['pktType'] == "Signal strength":
                            ...
                            if sd[0]-t1 <= 0.028:
                                ...
                                res.append([f"PTxDUT removes the power signal in Tterminate {round((sd[0]-t1)*1000,3)} ms after illegal SIG packet is received, Expected: <= 28 ms", "Pass"])
                            elif sd[0]-t1 <= 0:
                                res.append([f"Tterminate is 0 ms as Uro drops below 200 mV before the end of the illegal SIG data packet, Expected: <= 0 ms", "Pass"])
            ...
            else: res.append([f"Shutdown TesterMsg not found", "Inconclusive"])
            ...
            if len(sadt1)>2:
                ...
                if len(clk1)>2:
                    ...
                    if len(clk_exit)>2:
                        ...
                        if len(sadc_open2)>2:
                            res.append([f"SADC Open Stream packet found at {round(sadc_open2[0],3)} sec after cloak exit", "Pass"])
                            ...
                            if sadco2_resp is not None and self.file_list[sadco2_resp]['pktType'] == "SDSR" and "ACK" in self.file_list[sadco2_resp]['value']:
                                res.append([f"SDSR/ACK response found at {round(self.file_list[sadco2_resp]['startTime'],3)} sec, Expected: SDSR/ACK", "Pass"])
                            else:
                                SDSR_ACK_flag = False
                                res.append([f"{self.file_list[sadco2_resp]['pktType']}_{self.file_list[sadco2_resp]['value']} found at {round(self.file_list[sadco2_resp]['startTime'],3)} sec, Expected: SDSR/ACK", "Fail"])
                            ...
                            if len(sadt2)>2:
                                ...
                                if len(sadc_close2)>2:
                                    res.append([f"SADC Close Stream packet found at {round(sadc_close2[0],3)} sec after SADT", "Pass"])
                                    ...
                                    if sadccl2_resp is not None and self.file_list[sadccl2_resp]['pktType'] == "SDSR" and "ACK" in self.file_list[sadccl2_resp]['value']:
                                        res.append([f"SDSR/ACK response found at {round(self.file_list[sadccl2_resp]['startTime'],3)} sec, Expected: SDSR/ACK", "Pass"])
                                    else:
                                        SDSR_ACK_flag = False
                                        res.append([f"{self.file_list[sadccl2_resp]['pktType']}_{self.file_list[sadccl2_resp]['value']} found at {round(self.file_list[sadccl2_resp]['startTime'],3)} sec, Expected: SDSR/ACK", "Fail"])

                                    # CERTIFICATE
                                    sadc_open3 = self.PktMethod.GetPacketDetails(packet='SADC',value="Open Stream:",limit=[clk_exit[2], end],Type="Response")
                                    if len(sadc_open3)>2:
                                        res.append([f"SADC Open Stream Response found at {round(sadc_open3[0],3)} sec after cloak exit", "Pass"])
                                        
                                        sadt3 = self.PktMethod.GetPacketDetails(packet='SADT',limit=[sadc_open3[2], end],Type="Response")
                                        if len(sadt3)>2:
                                            res.append([f"SADT Response found at {round(sadt3[0],3)} sec", "Pass"])
                                            ...
                                            if cert_type3 == "Certificate":
                                                res.append([f"{cert_type3} found in SADT response at {round(sadt3[0],3)} sec", "Pass"])
                                            else:
                                                res.append([f"Invalid {cert_type3} found in SADT response at {round(sadt3[0],3)} sec", "Fail"])

                                            header_val = self.file_list[sadt3[2]]['header_Payload']['sFieldType'].split()[4].strip()
                                            print("header_val:", header_val)
                                            if header_val == "0x12":
                                                res.append([f"Certificate response started with header {header_val}, Expected: 0x12", "Pass"])
                                            else:res.append([f"Certificate response started with header {header_val}, Expected: 0x12", "Fail"])
                                        else:res.append([f"SADT Response not found", "Fail"])
                ...
                else: res.append([f"Cloak not found", "Fail"])
            else:res.append([f"SADT packet not found", "Fail"])
        else:res.append([f"SADC Open Stream packet not found", "Fail"])

        if SDSR_ACK_flag:
            res.append([f"PTx DUT responded with SDSR/ACK to all request messages in GET_CERTIFICATE", "Pass"])
        else:
            res.append([f"PTx DUT not responded with SDSR/ACK to all request messages in GET_CERTIFICATE", "Fail"])
            ...
            if len(pkt_exit)>2:
                ...
                if len(ideal_pkt)>2:
                    ...
                    if len(stable_pla2)>2:
                        # res.append([f"PLA_2 packet found at {round(stable_pla2[0],3)} sec after cloak exit", "Pass"])
                        ...
                        if Prect_chk2 >= EXCAP["Nominal_Power_Mode"]:
                            res.append([f"Power level reached to Nominal_Power_Mode potential load power i.e, {Prect_chk2}W at {round(stable_pla2[0],3)} sec, Expected: {EXCAP['Nominal_Power_Mode']}W", "Pass"])
                        else:
                            pass
                ...
                if len(MSRreq2)> 2:
                    ...
                    res.append([f"Main mode in MSR packet is {PrefMode2}, Expected: High_Power_Mode", "Pass" if PrefMode2 == "High_Power_Mode" else "Fail"])
                    ...
                    if len(MSS2)> 2:
                        ...
                        else: res.append([f"Load Set Done packet not observed","Fail"])
        return res
    ...
    def t_CAL_OP_PROC(self,Flow_limit,Check):
        ...
        while id < Flow_limit[1]:
            if 'CAL_OP' in self.file_list[id]['pktType'] and "CMT" in self.file_list[id]['value']:
                res.append([f"CAL_OP[COMMIT] packet found at {round(self.file_list[id]['startTime'],2)} sec", "Pass"])
                ...
                if respid is not None:
                    if 'CAL_OP_RSP' in self.file_list[respid]['pktType'] and ("ACCEPTED" or "ERR_MODE_VALIDATION_FAIL" in self.file_list[respid]['value']):
                        res.append([f"CAL_OP_RSP[{self.file_list[respid]['value']}] response found at {round(self.file_list[respid]['startTime'],2)} sec, Expected: CAL_OP_RSP[ACCEPTED] or CAL_OP_RSP[ERR_MODE_VALIDATION_FAIL]", "Pass"])
                        # t_CAL_OP_PROC
                    ...
                    else: res.append([f"{self.file_list[respid]['pktType']}[{self.file_list[respid]['value']}] response recevied at {round(self.file_list[respid]['startTime'],2)} sec, Expected: CAL_OP_RSP[ACCEPTED] or CAL_OP_RSP[ERR_MODE_VALIDATION_FAIL]", "Fail"])
                else: res.append([f"CAL_OP_RSP response not recevied", "Fail"])
    ...
    def MODEXCAPCheck(self,Flow_limit,Check):
        ...
        if ECAP == {'LPM': 1, 'NPM': 1, 'HPM': 0, 'CPM': 0}:
            ...
            if EXCAP["Nominal_Power_Mode"] >= 15:
                res.append([f"NPM Potential Load Power is {EXCAP["Nominal_Power_Mode"]} W, Expected: >= 15 W", 'Pass'])
            else: res.append([f"NPM Potential Load Power is {EXCAP["Nominal_Power_Mode"]} W, Expected: >= 15 W", 'Fail']) 
            if GMP["G_NPM_CO"] != 0:
                res.append([f"G_NPM_CO is {GMP["G_NPM_CO"]} , Expected: != 0", 'Pass'])
            else: res.append([f"G_NPM_CO is {GMP["G_NPM_CO"]} , Expected: != 0", 'Fail'])
            if GMP["G_HPM_CO"] != 0:
    ...
    def Txce_interval(self,Flow_limit,Check):
        ...
        if len(dumppkt)>2:
            res.append([f"Load dump 400mA found at {self.PktMethod.Timeconvert(dumppkt[0])}", "Pass"])
            id = dumppkt[2]
                ...
                while id < end:
                    ...
                    if len(TempPkt1) > 2:
                        ...
                        if len(TempPkt2) > 2:
                            ...
                            id = TempPkt1[2]
                    id += 1
                res.append([f"MPP_XCEV_Ideal stabilization found at {self.PktMethod.Timeconvert(stable_pkt[0])}", "Pass"])
                # print("xcecnt:",len(set(xceids)))
                res.append([f"Total number of XCE packets received: {len(set(xceids))}", "Pass"])
                res.append([f"After subtracting 5 from the count of the XCE data packets", "Pass"])
            ...
            else: res.append([f"MPP_XCEV_Ideal stabilization not found.", "Fail"])
        else: res.append([f"Load dump 400mA not found.", "Fail"])
        return res
