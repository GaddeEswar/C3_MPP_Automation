...
class CTSChecks_MPP_TPR4():
    ...
    def _fmt_header(self):
        h = self.Header
        return (
            f"  Testcase  : {TestCaseConfig.TestcaseName} ({TestCaseConfig.TestcaseID})\n"
            f"  DUT       : {h.get('DUTName')} ({h.get('DUTID')}) | Board: {h.get('BoardNo')} | Coil: {TestCaseConfig.Coil}\n"
            f"  Run       : {h.get('Run')} | Cert: {h.get('Certification')} | SW: {h.get('SWVersion')} | FW: {h.get('FWVersion')}\n"
            f"  Result    : TC={TestCaseConfig.AutomationResult}  SW={TestCaseConfig.SoftwareResult} | Engineer: {h.get('Engineer')}"
        )
