...
class CommonCTSChecks:
    ...
    def XCE_Measurement(self,CTSCheck,Check,flows,flwID):
        ...
        if len(PT)>2:
            ...
            PktsCount=0
            res.append([f'PRx {''if PchPkt else 'did not' } sent PCH data packet . Tdelay was set to {PchTime} mS','Pass'])
                ...
                if len(pkt1)>2:
                    ...
                    if nextid is not None:
                        if 'XCEP_INTERVAL' in self.Header['TestcaseID']:
                            ...
                            # result= 'Pass' if TDC+PchTime>=PchTime+24+19 else 'Fail'
            ...
            if 'XCEP_INTERVAL' in self.Header['TestcaseID']:
                ...
                if FailCount >  int(0.05 * len(Timings)):res.append([f'More than 5% of the Intervals met the fail criteria', "Fail"])
                else:res.append([f'More than 95% of the Intervals met the Pass criteria', "Pass"])
                res.append([f'Measured  Max {Check['TimingCheck']} from XCE at {max_item[1]}  to XCE at {max_item[2]}  is {max_item[0]} mS , Min {Check['TimingCheck']} from XCE at {min_item[1]}  to XCE at {min_item[2]}  is {min_item[0]} mS Limit : <={Check['Limit'][1] }', "Pass"])

            if 'XCEP_HANDLING' in self.Header['TestcaseID']:
                min_item = min(Handling, key=lambda x: x[0])
                max_item = max(Handling, key=lambda x: x[0])
                res.append([f'Measured  Max Interval -- txce_responsetimeout + tdelay + tcontrol from {max_item[1]} Sec to {round(max_item[2],3)} Sec is {max_item[0]} mS , Limit : >={PchTime+24+19 }', "Pass" if PchTime+24+19 else 'Fail'])
                res.append([f'Measured  Min Interval -- txce_responsetimeout + tdelay + tcontrol from {min_item[1]} Sec to {round(min_item[2],3)} Sec  is {min_item[0]} mS , Limit : >={PchTime+24+19 }', "Pass" if PchTime+24+19 else 'Fail'])

            res.append([f'PRx sent {PktsCount} {Check['Pkt'][0]} data packets Expected :{Check['PktsCount']} Pkts', "Inconclusive" if PktsCount< Check['PktsCount'] else 'Pass'])
        ...
        if len(PT)>2:
            ...
            while id < self.Flow_limit[1]:
                ...
                if len(pkt1)>2:
                    ...
                    if Response is not None:res.append([f'TPT sent Response for the {Check['Pkt'][0]} Packet at {{{pkt1[2]}}} is {Response[0]}', "Inconclusive" if Response[0] !="ACK" else 'Pass'])  
                    else:res.append([f'TPT did not sent response for the {Check['Pkt'][0]} Packet at {{{pkt1[2]}}}','Inconclusive']) 
                    nextid=self.findTypeid(limit=[pkt1[2]+1,self.Flow_limit[1]],Type='Packet')
                    if nextid is not None :
                        TDC=round((self.file_list[nextid]['startTime']-self.file_list[pkt1[2]]['stopTime'])*1000,2)
                        Timings.append(TDC)
                        result= 'Pass' if TDC>=44 else 'Fail'
                        res.append([f'Measured Timing between {Check['Pkt'][0]} at {{{pkt1[2]}}} and Next Packet at {{{nextid}}} is {TDC} mS  Limit: >= 44 mS', result])
                        id=nextid
                    else:break
                else:break
            #min max average.
            res.append([f"Obtained min time: {min(Timings)} and max time: {max(Timings)}", "Pass"])
            if PktsCount< Check['PktsCount']:res.append([f'PRx sent {PktsCount} {Check['Pkt'][0]} data packets only', "Inconclusive"])
        else:res.append([f'PRx did not Entered PT Phase','Inconclusive'])
        return res
    ...
    def PLA_Slow_Mode(self,CTSCheck,Check,flows,flwID):
        ...
        if len(pings_360)>=6:
            ...
            for ping in pings_360:               
                if count == 0: res.append([f"Main Test", "Pass"])
                elif count != 0: res.append([f'Repeat -- {count}','Pass'])
                ...
                if len(Nego)>2:
                    ...
                    if len(pkt1)>2:
                        ...
                        if len(pkt2)>2:
                            ...
                            if Response is not None: res.append([f'TPT sent Response for the {Check['Pkt2'][0]} at {{{pkt1[2]}}} is {Response[0]}', "Inconclusive" if Response[0] !="ACK" else 'Pass']) 
                            else:res.append([f'TPT did not sent response for the {Check['Pkt2'][0]} at {{{pkt1[2]}}}','Inconclusive'])              
                            res.append([f'Measured Timing between {Check['Pkt1'][0]}_{Check['Pkt1'][1]} at {{{pkt1[2]}}} and PLA at {{{pkt2[2]}}} is {Timing} mS  Limit: <={Check['limit'][1]} mS', 'Pass' if Timing > Check['limit'][0] and Timing <= Check['limit'][1] else 'Fail'])
                        else:res.append([f'PRx did not sent {Check['Pkt1'][0]} in the Count :{count}','Inconclusive'])
                    else:res.append([f'PRx did not sent {Check['Pkt1'][0]}_{Check['Pkt1'][1]} in the Count :{count}','Inconclusive'])
                else:res.append([f'PRx did not Entered Nego Phase in the Count :{count}','Inconclusive'])
                count += 1
        else:res.append([f'TPT did not sent Enough Pings for measurements','Inconclusive'])
        return res
            ...
            while id < self.Flow_limit[1]:
                ...
                if len(pkt1)>2:
                    ...
                    if Response is not None:
                        result='Pass' if 'ACK' in Response[0] and PktsCount <8 else 'Pass' if Check['Response'] in Response[0] and PktsCount >7 else 'Inconclusive'
                        res.append([f'TPT sent Response for the {Check['Pkt'][0]} at {{{pkt1[2]}}} is {Response[0]}', result])  
                    else:
                        if Check['Response'] is None and PktsCount>7:res.append([f'TPT did not sent response for the {Check['Pkt'][0]} at {{{pkt1[2]}}}','Pass'])
                        else:res.append([f'TPT did not sent response for the {Check['Pkt'][0]} at {{{pkt1[2]}}}','Inconclusive'])     
                    pkt2= self.PktMethod.GetPacketDetails(packet=Check['Pkt'][0],limit=[pkt1[2]+1,self.Flow_limit[1]])
                    if len(pkt2)>2:
                        if PktsCount >7:
                            Timing=round((self.file_list[pkt2[2]]['startTime']-self.file_list[pkt1[2]]['startTime'])*1000,2)
                            res.append([f'Measured Timing between {Check['Pkt'][0]} at {{{pkt1[2]}}} and {Check['Pkt'][0]} at {{{pkt2[2]}}} is {Timing} mS  Limit: <={Check['limit'][1]} mS', 'Pass' if Timing > Check['limit'][0] and Timing <= Check['limit'][1] else 'Fail'])
                        id=pkt2[2]
                    else:break
                else:break
            if PktsCount< Check['PktsCount']:res.append([f'PRx sent {PktsCount} {Check['Pkt'][0]} data packets only', "Inconclusive"])
    ...
    def ATN_Handling(self,CTSCheck,Check,flows,flwID):
        ...
        if len(pings_360)>=7:
            ...
            for ping in pings_360:
                self.Flow_limit=ping
                if count != 0: res.append([f'Repeat -- {count}','Pass'])
                else: res.append([f"Main Test", "Pass"])
                PT=self.PktMethod.GetPacketDetails(packet="Phase_Info",value="Phase Type: PT", Type="TesterMsg",limit=self.Flow_limit)
                if len(PT)>2:
                    ...
                    if len(Stable)>2:
                        # find Pkt
                        res.append([f'PRx Stabilized at {{{Stable[2]}}}','Pass'])
                        ...
                        if len(pkt1)>2:
                            ...
                            if len(pkt2)>2:
                                ...
                                res.append([f'Measured Timing between ATN Response at {{{pkt1[2]}}} and DSR_POLL at {{{pkt2[2]}}} is {Timing} mS  Limit:<= 500 mS', 'Pass' if Timing > 0 and Timing <= 500 else 'Fail'])
                            else:res.append([f'PRx did not sent DSR_POLL Packet in the Count :{count}','Inconclusive'])
                        else:res.append([f'TPT did not sent ATN Response in the Count :{count}','Inconclusive'])
                    else:res.append([f'PRx did not stabilized in the Count :{count}','Inconclusive']) 
                else:res.append([f'PRx did not Entered PT Phase in the Count :{count}','Inconclusive'])
                count+=1
        else:res.append([f'TPT did not sent Enough Pings for measurements','Inconclusive'])
        return res
    ...
    def IllegalPackets(self,CTSCheck,Check,flows,flwID):
        ...
        if len(PT)>2:
            id=PT[2]
            res.append([f'PRx entered in to MPP Full Mode power transfer phase at {{{PT[2]}}}','Pass'])
            IllegalPkts=False
            EndPower=False
            count=0
            CEpkts=True
            while id < self.Flow_limit[1]:
                ...
                if self.file_list[id]['pktType'] in Check['Pkts']:
                    IllegalPkts=True
                    res.append([f'PRx sent the Data Packet :{self.file_list[id]['pktType']} at {{{id}}}','Fail'])
                if self.file_list[id]['pktType'] in ["Extended Control Error", "Power Loss Accounting"]:
                    ...
                    if Response is not None:
                        if "NAK" in Response[0]:
                            res.append([f'TPT sent NAK Response for the {self.file_list[id]['pktType']} Pkt at  {{{id}}}','Inconclusive']) 
                            CEpkts=False
                    else:  res.append([f'TPT did not sent Response  for the {self.file_list[id]['pktType']} Pkt at  {{{id}}}','Inconclusive']) 
                    if self.file_list[id]['pktType']=="Extended Control Error":count+=1
                if self.file_list[id]['pktType'] in ['End Power Transfer']:
                        res.append([f'PRx sent End Power Transfer Pkt at {{{id}}}','Pass'])
                        EndPower=True
                        break
                id+=1
            if CEpkts:res.append([f'TPT sent ACK Response for all XCE and PLA Packets','Pass'])
            if not IllegalPkts:res.append([f'PRx did not sent any Packets in the List :{Check['Pkts']}','Pass'])
            if not EndPower :res.append([f'PRx sent {count} XCE Packets Expected :>=200','Pass' if count >=200 else 'Inconclusive'])
        else:res.append([f'PRx did not Entered MPP Full Mode power transfer phase','Inconclusive'])
        return res
    def TnegTransition(self,CTSCheck,Check,flows,flwID):
        ...
        if len(MPP)>2:
            ...
            if RestrictedBit==1:
                PchTime,PchPkt=self.PchTime()
                res.append([f'PRx set Restricted Bit to One in the MPP-Extended Identification Packet at {{{MPP[2]}}}','Pass'])
                res.append([f'PRx {''if PchPkt else 'did not' } sent PCH data packet with Power Control Hold Off Time:{PchTime} mS','Pass'])
                End_Power=self.PktMethod.GetPacketDetails(packet="End Power Transfer", value= "[EPT/rst]",limit=[MPP[2]+1,self.Flow_limit[1]])
                if len(End_Power)<2:
                    res.append([f'PRx did not sent End Power Tranfer Packet _RST ','Pass'])
                    # find Renegotiate
                    Renego=self.PktMethod.GetPacketDetails(packet="Renegotiate", limit=[MPP[2],self.Flow_limit[1]])
                    if len(Renego)<2:res.append([f'PRx did not sent Renegotiate Packet to Switch from Restricted Mode to MPP-Full Mode','Inconclusive'])
                    else:
                        ...
                        if len(SRQ_En)<2:res.append([f'PRx did not Completed Renegotiation Sequence by not sending SRQ/En packet','Inconclusive'])
                        else:
                            ...
                            if Response is not None:
                                res.append([f'TPT sent Response for the SRQ/End Negotiation at {{{SRQ_En[2]}}} is {Response[0]}', "Inconclusive" if Response[0] !="ACK" else 'Pass'])  
                                Timing=round((self.file_list[Response[1]]['stopTime']-Renego[1])*1000,2)
                                res.append([f'Measured TnegTransition Timing from end of Packet at {{{Renego[2]}}} to Id at {{{Response[1]}}} is {Timing} mS Limit:<={Check['limit'][1]}','Pass' if Timing >Check['limit'][0] and Timing <= Check['limit'][1] else 'Fail'])
                            else:res.append([f'TPT did not sent response for the SRQ/End Negotiation at {{{SRQ_En[2]}}}','Inconclusive']) 
                else:res.append([f'PRx sent End Power Tranfer Packet _RST to Switch from Restricted Mode to MPP-Full Mode','Pass'])
            else:res.append([f'PRx did not set Restricted Bit to One in the MPP-Extended Identification Packet','Inconclusive'])
        else:res.append([f'PRx did not sent MPP-Extended Identification Packet','Inconclusive'])
        return res
    def EPTR(self,CTSCheck,Check,flows,flwID):
        ...
        if len(Stable)>2:
            res.append([f'PRx Stablized at {{{Stable[2]}}}','Pass'])
            ...
            if len(xce)>2:
                ...
                if Response is not None:
                    res.append([f'TPT sent Response --{Response[0]} for the XCE Packet at {{{xce[2]}}}','Pass' if Response[0]=="ATN" else 'Inconclusive'])  
                else:res.append([f'TPT did not sent response for the XCE Packet at {{{xce[2]}}}','Inconclusive']) 
                # check DSRPOll
                ...
                if len(DSR)>2:
                    ...
                    if Response is not None:
                        res.append([f'TPT sent Response as {Response[0]} for the DSR_POLL Packet at {{{DSR[2]}}}','Pass' if Response[0]=="EPTR [0x0A] " else 'Inconclusive'])
                            # find EPT Packet
                        End_Power=self.PktMethod.GetPacketDetails(packet="End Power Transfer", value= "[EPT/pmc]",limit=[Response[1]+1,self.Flow_limit[1]])
                        if len(End_Power)<2:
                            res.append([f'PRx did not sent EPT/pmc Packet','Fail'])
                        else:
                            res.append([f'PRx sent EPT/pmc Packet at {{{End_Power[2]}}}','Pass'])
                            Timing=round((End_Power[0]-self.file_list[Response[1]]['startTime'])*1000,2)
                            res.append([f'Measured t_eptc Timing from end of Packet at {{{Response[1]}}} to Id at {{{End_Power[2]}}} is {Timing} mS Limit:<={Check['limit'][1]}','Pass' if Timing >Check['limit'][0] and Timing <= Check['limit'][1] else 'Fail'])
                    else:res.append([f'TPT did not sent Response  for the DSR_POLL Packet at {{{DSR[2]}}}', 'Inconclusive'])
                else:res.append([f'PRx did not sent DSR_POLL Packet ','Inconclusive'])
            else:res.append([f'Test did not found XCE-ATN sequence after Stabilization at {{{Stable[2]}}}','Inconclusive']) 
        else: res.append([f'PRx did not stabilized ','Inconclusive'])
        return res
    ...
    def CAL_Pending(self,CTSCheck,Check,flows,flwID):
        ...
        if len(Cal_CMT)>2:
            res.append([f'PRx sent CAL_OP [COMMIT] data packet at {{{Cal_CMT[2]}}}','Pass'])
            # Check response for the CAL COMMIT
            Response=self.PktResponse(Cal_CMT[2]+1,self.Flow_limit[1])
            if Response is not None:
                result="Pass" if "CAL_OP_RSP [0x1B]" in self.file_list[Response[1]]['pktType'] and "Status: PENDING" in self.file_list[Response[1]]['value'] else 'Inconclusive'
                ...
                if len(Cal_Exit)<2:res.append([f'PRx did not sent CAL[Exit] data packet ','Inconclusive'])
                else:
                    ...
                    while id < Cal_Exit[2]:
                        ...
                        if len(Cal_CMT_Next)>2:
                            Pktfound=True
                            res.append([f'PRx sent CAL_OP [COMMIT] data packet at {{{Cal_CMT_Next[2]}}}','Pass'])
                            Response=self.PktResponse(Cal_CMT_Next[2]+1,self.Flow_limit[1])
                            if Response is not None:
                                result="Pass" if "CAL_OP_RSP [0x1B]" in self.file_list[Response[1]]['pktType'] and "Status: ACCEPTED" in self.file_list[Response[1]]['value'] else 'Inconclusive'
                                res.append([f'TPT  sent Response as {Response[0]}_{self.file_list[Response[1]]['value']} for the CAL_OP[COMMIT] Packet at {{{Cal_CMT[2]}}}', result])
                            else:res.append([f'TPT did not sent Response  for the CAL_OP [COMMIT] Packet at {{{Cal_CMT[2]}}}', 'Inconclusive'])
                            id=Cal_CMT_Next[2]+1
                        else:break
                    if not Pktfound:
                                        ...
                                        if 'DSR' in self.file_list[Pktid]['pktType'] and 'POLL' in self.file_list[Pktid]['value']:
                                            ...
                                            if TPTid is not None:
                                                if "CAL_OP_RSP [0x1B]" in  self.file_list[TPTid]['pktType'] and "Status: ACCEPTED" in self.file_list[TPTid]['value']:
                                                    res.append([f'TPT sent CAL_OP_RSP Accepted Respponse at {{{TPTid}}} for ATN-DSR/POLL Sequence','Pass'])
                                                    seqFound=True
                                                i=TPTid+1
                                            else:break
                                        i=Pktid+1
                        ...
                        if not seqFound:res.append([f'PRx did not sent the CAL_OP [COMMIT] Packets before CAL EXIT PKt at {{{Cal_Exit[2]}}}', 'Inconclusive'])
                    res.append([f'PRx sent CAL[Exit] data packet at {{{Cal_Exit[2]}}}','Pass'])
            else:res.append([f'TPT did not sent Response  for the CAL_OP COMMIT] Packet at {{{Cal_CMT[2]}}}', 'Inconclusive'])
        else:res.append([f'PRx did not sent CAL_OP [COMMIT] data packet ','Inconclusive'])
        return res
    ...
    def Q_Deflection(self,CTSCheck,Check,flows,flwID):
        ...
        res.append([f'Measured Q-Open is {round(Q_Updated_open,3)}','Pass' ])
        res.append([f'Measured Q-PRx is {round(Q_Updated_Mated,3)}','Pass' ])
        res.append([f'Measured Q-Deflection is {Q_Deflection} % , Expected :>20%','Pass' if Q_Deflection >20 else 'Fail'])
        return res
    ...
    def DPLoss_Valid(self,CTSCheck,Check,flows,flwID):
        ...
        if len(PT)>2:
            ...
            if len(CAL_ENTER)>2:
                res.append([f'TPT sent CAL_ENTER_RSP data packet at {{{CAL_ENTER[2]}}} ','Pass'])
                Cal_Exit=self.PktMethod.GetPacketDetails(packet="CAL_EXIT [0x2D]", limit=[CAL_ENTER[2],self.Flow_limit[1]])
                if len(Cal_Exit)<2:res.append([f'TPT did not sent CAL[Exit] data packet ','Inconclusive'])
                else:
                    ...
                    if len(ECAP)>2:
                        ...
                        res.append([f'TPT set Negotiable Load power to {NPower} W in ECAP Packet Expected val :{NPowerCal} W. In SDF Guranteed Load Power was set to {GPower} W ','Pass' if NPower== NPowerCal else 'Inconclusive'])   
                    else: res.append([f'Test did not found ECAP data pkt', 'Inconclusive'])
                    res.append([f'PRx sent CAL[Exit]  data packet at {{{Cal_Exit[2]}}} ','Pass'])
                    #Measure Tcal Timing
                    Tcal=round((Cal_Exit[0]-CAL_ENTER[1])*1000,2)
                    res.append([f'Measured Tcal is {round(Tcal/(1000*60),3)} mins Expected <=5Mins','Pass' if Tcal <=300000 else 'Fail'])
                    PrectVals=self.PrectValues(limit=[CAL_ENTER[2],Cal_Exit[2]])
                    res.append([f'No of Calibrations Points Received is {len(PrectVals)} Expected >=60','Pass' if len(PrectVals)>=60 else 'Fail'])
                    # Group PRECT Values
                    Levels=self.Group_PRECT(PrectVals)
                    for Level in Levels.keys():
                        res.append([f'No of Points Received in PRECT LEVEL {Level} is {len(Levels[Level])} Expected >=18','Pass' if len(Levels[Level])>=18 else 'Fail'])
                    res.append([f'Measured PRECT_Max is {max(PrectVals)} W and PRECT_Min is {min(PrectVals)} W . The Difference is {round((max(PrectVals)-min(PrectVals)),3)} W Expected >=5W','Pass' if (max(PrectVals)-min(PrectVals))>=5 else 'Fail'])
                    ...
                    if len(Stable)>2:
                        ...
                        for val in DplossVals:
                            if -385.0 < val*1000 <= 385.0:continue
                                ...
                                DPlossFlag=False
                                res.append([f'Measured ΔPLOSS Value is {val} Limit :-385 mW to +385 mW','Fail'])
                        if DPlossFlag: res.append([f'Measured Max ΔPLOSS Value is {max(DplossVals)} and min ΔPLOSS Value is {min(DplossVals)} Limit :-385 mW to +385 mW','Pass'])
                            ...
                            while pid < self.Flow_limit[1]:
                                ...
                                if len(TPLA)>2:
                                    ...
                                    if TPRECT  < NpLimit[0]  or TPRECT > NpLimit[1]:
                                        res.append ([f'Observed PRECT power  in PLA2 Packet at {{{pid}}} is {TPRECT}W Expected Target Load : {NpLimit[0]} W -{NpLimit[1]} W ','Inconclusive'])
                                        FalseFlag=True
                                    pid=TPLA[2]+1
                                else:break
                            if not FalseFlag:res.append([f'ALL PRECT Values in PLA2 Packets are within 10% of  Target Load power : {NPowerCal} W i.e {NpLimit[0]} W ~ {NpLimit[1]} W after the Stabilization  ','Pass'])
                        except Exception as e:
                            res.append([f'Error while Check Target Load : {e}', 'Inconclusive'])

                        # Check run Time
                        RunTime=round((self.file_list[self.Flow_limit[1]]['startTime']-Stable[1])*1000,3)
                        res.append([f'Test Executed for {round(RunTime/(1000*60),3)} mins after Stabilization at {{{Stable[2]}}}, Exp :> 5mins','Inconclusive' if RunTime <300000 else 'Pass'])

                    else:res.append([f'PRx did not stabilized ','Inconclusive'])
            else:res.append([f'PRx did not Started DPLOSS Calibration by not sending CAL ENTER Pkt ','Inconclusive'])
        else:res.append([f'PRx did not Entered PT Phase','Inconclusive'])
        return res
        ...
        if len(KestMsg)>2:
            ...
            if Check['Response']:
                ...
                if len(CFG)>2:
                    ...
                    if Response is not None:
                        res.append([f'TPT sent Response :{Response[0]} for the CFG data packet at {{{CFG[2]}}} ','Pass' if "NAK" in Response[0] else "Inconclusive"])  
                        # Check KEST Pkt
                        KEST=self.PktMethod.GetPacketDetails(packet="KEST-COEFF [0x50]", limit=[CFG[2]+1,len(self.file_list)-1])
                        res.append([f'PRx {"" if len(KEST)>2 else "did not"} sent KEST-COEFF [0x50] data Packet {f"at {{{KEST[2]}}}" if len(KEST)>2 else ''}','Pass' if len(KEST)>2 else "Inconclusive"])
                    else:res.append([f'TPT did not sent Response for the CFG data packet at {{{CFG[2]}}} ','Inconclusive'])
                else:res.append([f'PRx did not sent Configuration data packet ','Inconclusive'])
            res.append([f'Measured K_est Val:{KestVal} , SDF Ki_Actual is :{Kest} ','Pass'])   
            res.append([f'Calculated K_est Error is :{K_Error} , Limit : <0.06 ','Pass' if K_Error <0.06 else 'Fail'])   
        else:res.append([f'Did not found Kest Val in the Sequence','Inconclusive'])
        return res
    ...
    def Load_RNEG(self,CTSCheck,Check,flows,flwID):
        ...
        PT=self.PktMethod.GetPacketDetails(packet="Phase_Info",value="Phase Type: PT", Type="TesterMsg",limit=self.Flow_limit)
        if len(PT)>2:
            res.append([f'PRx entered Power Transfer Phase at {{{PT[2]}}}','Pass'])
            Stable=self.PktMethod.GetPacketDetails(packet="Stabilized",Type="TesterMsg",limit=[PT[2],self.Flow_limit[1]])
            if len(Stable)>2:
                res.append([f'PRx Control Stabilized at {{{Stable[2]}}}','Pass'])
                ...
                if len(Throttle_Start)>2:
                    ...
                    if len(Throttle_End)>2:
                        ...
                        res.append([f'Measured Avearge Rectified Power Prect_Initial is {PrectInitial} W for 10 PLA Packets','Pass'])
                        res.append([f'Throttle started at {{{Throttle_Start[2]}}}','Pass'])
                        # Check NAK Response for the PLA2 and XCE Packets
                        res.append([f'--------------- Verify The NAK Response from TPT during Throttle ---------------','Pass'])
                        ...
                        res.append([f'Throttle Ended at {{{Throttle_End[2]}}}','Pass'])
                        ...
                        if len(Stable2)>2:
                            res.append([f'PRx Stabilized  at {{{Stable2[2]}}} after Throttle','Pass'])
                            ...
                            if len(ECAP)<2:res.append([f'ECAP packet did not found after Throtlling','Inconclusive'])
                            else:
                                res.append([f'TPT sent ECAP Packet at {{{ECAP[2]}}} after Throtlling', 'Pass'])
                                NPower=float((self.PktMethod.hex_to_decimal(self.PktMethod.GetPayloadDetails(ECAP[2],'Negotiable_Load_Power')[0]['sRawData']))/10)
                                res.append([f'TPT set Negotiable Power to {NPower} W in ECAP packet','Pass' if abs(NPower-(PrectInitial/2))<0.5 else 'Inconclusive'])
                                # Check ACK Response for the PLA2 and XCE Packets
                                res.append([f'--------------- Verify The ACK Response from TPT after Renegotiation ---------------','Pass'])
                                ...
                                res.append([f'Power {"did not" if len(Stable3)<2 else ''} Stabilized after ReNegotaition Phase ','Inconclusive' if len(Stable3)<2 else 'Pass'])  
                        else:res.append([f'Power did not Stabilized after Throttling ','Inconclusive'])
                else:res.append([f'Throttling has not been Initialized','Inconclusive'])
            else:res.append([f'PRx did not stabilized to Negotiable Load Power','Inconclusive'])
        else:res.append([f'PRx did not Entered PT Phase','Inconclusive'])
        return res
    ...
    def MatedQ(self,CTSCheck,Check,flows,flwID):
        ...
        if len(MatedQ)>2:
            ...
            res.append([f'PRx sent MATEDQ-COEFF [0xA8]  data packet with Prameters :{Vals} ','Pass'])
            # find TX MatedQ
            ...
            if len(MatedQRes)>2:
                ...
                res.append([f'PTx sent MATEDQ Result :{value} ','Pass' if "FO detected" in value else "Fail"])
            else:res.append([f'PTx did not sent MATEDQ Result ','Inconclusive'])  
            ...
            #     res.append([f'PTx sent MATEDQ_RES [0x40]  data packet with Result :{Value} ','Pass' if "Foreign Object Detected" in Value else "Fail"])
            # else:res.append([f'PTx did not sent MATEDQ_RES [0x40]  data packet ','Inconclusive'])  
        else:res.append([f'PRx did not sent MATEDQ-COEFF [0xA8]  data packet ','Fail'])
        return res
        ...
        if len(CFG)>2:
            ...
            if Response is not None:
                ...
                if len(Get_Error)>2:
                    ...
                    if Response is not None:
                        res.append([f'TPT sent Response :{Response[0]} for the GET[Error Status] data packet at {{{Get_Error[2]}}} ','Pass' if Response[0]=="PTx Error status" else "Inconclusive"])
                        result=self.Payload_Details(PacketName=Response[0],Index=Response[1],PayLoads=Check['PTx'],Receiver=False)  
                        if len(result)>0:res.extend(result)   
                    else:res.append([f'TPT did not sent Response for the a GET[Error Status] data packet at {{{Get_Error[2]}}} ','Inconclusive'])
                ...
                if 'MPP1' in self.Header['Coil']:
                    ...
                    res.append([f'PRx {"" if len(EPT)>2 else "did not"} sent EPT/rst data Packet {f"at {{{EPT[2]}}}" if len(EPT)>2 else ''}','Pass' if len(EPT)>2 else "Fail"])
                    ...
                    res.append([f'PRx {"" if len(SRQ_En)>2 else "did not"} sent SRQ/en data Packet {f"at {{{SRQ_En[2]}}}" if len(SRQ_En)>2 else ''} ','Fail' if len(SRQ_En)>2 else "Pass"])
                else:
                    ...
                    if len(KEST)>2:
                        ...
                        if len(result)>0:res.extend(result)  
                    else:res.append([f'PRx did not sent KEST_COEFF data packet ','Fail'])
            else:res.append([f'TPT did not sent Response for the CFG data packet at {{{CFG[2]}}} ','Inconclusive'])
        else:res.append([f'PRx did not sent Configuration data packet ','Inconclusive'])
        return res
    ...
    def SwitchingReset(self,CTSCheck,Check,flows,flwID):
        ...
        if len(pings_128)>=2:
            ...
            if len(SRQ_FS)>2:
                ...
                if Response is not None:res.append([f'TPT sent Response :{Response[0]} for the  SRQ/freqsel data packet at {{{SRQ_FS[2]}}} ','Pass' if Response[0]=="ACK" else "Inconclusive"])
                else:res.append([f'TPT did not sent Response for the SRQ/freqsel  data packet at {{{SRQ_FS[2]}}} ','Inconclusive'])
            else:res.append([f'PRx did not sent SRQ/freqsel data packet ','Inconclusive'])
            #Check CFG Response and BPP packets
            CFG=self.PktMethod.GetPacketDetails(packet="Configuration", limit=ping2)
            if len(CFG)>2:
                res.append([f'TPT removed Power signal at {{{ping1[1]}}} and re-enters Ping Phase at {{{ping2[0]}}}','Pass'])
                Response=self.PktResponse(CFG[2]+1,ping2[1])
                if Response is not None:
                    res.append([f'TPT sent Response :{Response[0]} for the CFG data packet at {{{CFG[2]}}} ','Pass' if "ACK" in Response[0] else "Inconclusive"])
                    # Check BPP Pkts after CFG
                    CE=self.PktMethod.GetPacketDetails(packet="Control Error", limit=ping2)
                    res.append([f'PRx {"" if len(CE)>2 else "did not"} entered BPP Phase {f"at {{{CE[2]}}}" if len(CE)>2 else ''}','Pass' if len(CE)>2 else "Fail"])
                else:res.append([f'TPT did not sent Response for the CFG data packet at {{{CFG[2]}}} ','Inconclusive'])   
            else:res.append([f'PRx did not sent Configuration data packet in the Flow','Inconclusive'])
        else:res.append([f'Test did not found sufficient pings to validate Measurements','Inconclusive'])
        return res
        ...
        if PhaseLimit is not None:
            if Check['flow']==1:
                ...
                if len(SRQ_FS)>2:
                    ...
                    if len(result)>0:res.extend(result)       
                else:res.append([f'PRx did not sent SRQ/freqsel Pkt','Fail'])
                # Check End Negotiation is not present
                SRQ_En=self.PktMethod.GetPacketDetails(packet="SRQ [0x20] ",value="End Negotiation", limit=PhaseLimit)
                res.append([f'PRx {"" if len(SRQ_En)>2 else "did not"} sent SRQ/en data Packet {f"at {{{SRQ_En[2]}}}" if len(SRQ_En)>2 else ''} ','Pass' if len(SRQ_En)>2 else "Fail"])
                
                EPT=self.PktMethod.GetPacketDetails(packet="End Power Transfer",value="[EPT/rep]", limit=self.Flow_limit)
                res.append([f'PRx {"" if len(EPT)>2 else "did not"} sent EPT/rep data Packet {f"at {{{EPT[2]}}}" if len(EPT)>2 else ''}','Pass' if len(EPT)>2 else "Fail"])
            else:
                # check Nego Pkts
                result=self.NegoValidate(PhaseLimit,Check['NegPkts'])
                if len(result)>0:
                    res.append([f'----------- Nego Phase Validation -----------','Pass'])
                    res.extend(result)
                result=self.CheckPkts(PhaseLimit[0],PhaseLimit[1],Check['Pkts'])
                if len(result)>0:
                    res.append([f'----------- Pass Criteria Check ------------','Pass'])
                    res.extend(result)

        else:res.append([f'PRx did not entered in to Nego Phase','Inconclusive'])
        return res
    ...
    def Response_Valid_Device(self,CTSCheck,Check,flows,flwID):
        ...
        if PhaseLimit is not None:
            if Check.get("Cloak",False): # Check Entered to Cloak Phase or not
                ...
                if len(Cloak)>2:
                    res.append([f'PRx Entered in to Cloak Mode', 'Pass'])
                else:
                    ...
                    if len(PowerVals)>0:
                        ...
                        if AveragePFO==-1:res.append([f'TPT Cannot Calculate the PFO', 'Fail'])
                        else:
                            ...
                            # if Check.get("TestLink",False): # Check Average PFO form Other Tcs
                                ...
                                if TCExist:res.append([f'Average PFO calculated in the current Tc is {AveragePFO} mW --- for the {pkts} {Check['Pkt'][0]} Packets and max PFO value from {self.JCTSData[self.Product][self.Mode][self.Header['TestcaseID']][TestKey]["TestLink"]} is {max(TCPFO)} mW', 'Pass' if AveragePFO >= max(TCPFO) else 'Fail'])  
                                else:res.append([f'Average PFO measurements of TCs { self.JCTSData[self.Product][self.Mode][self.Header['TestcaseID']][TestKey]["TestLink"]} are not available ', 'Inconclusive'])
                            else:res.append([f'Calculated Average PFO Value for the {pkts} {Check['Pkt'][0]} Packets  is {AveragePFO} mW  Expected >=0mW', 'Pass' if AveragePFO >=0 else 'Fail'])                    
            else:
        ...
        if PhaseLimit is not None:
            ...
            if len(PT)>2:
                ...
                if len(Stable)>2:
                    ...
                    if len(SRQ_VS)>2:
                        res.append([f'PRx sent SRQ/Version select Pkt at {{{SRQ_VS[2]}}} in Negotiation Phase','Pass' if Check['VersionSelect'] else 'Fail'])
                        if Check['VersionSelect']:
                            ...
                            result=self.CheckPkts(PhaseLimit[0],PhaseLimit[1],Check['Pkts'])
                            if len(result)>0:
                                res.append([f'--- Validate Nego Packetes ---','Pass'])
                                res.extend(result)
                    else:res.append([f'PRx did not sent SRQ/Version select Pkt in Negotiation Phase','Pass' if not Check['VersionSelect'] else 'Fail'])
                else:res.append([f'PRx did not stabilized to Negotaible Load Power ','Inconclusive'])
            else:res.append([f'PRx did not entered in to PT Phase','Inconclusive'])
        else:res.append([f'PRx did not entered in to Nego Phase','Inconclusive'])
        return res
        ...
        if len(pings_360)>2:
            ...
            for ping in pings_360:
                ...
                if len(PLAP)>2:
                    ...
                    res.append([f'TPT sent {Check['Pkt'][0]} at {{{PLAP[2]}}} with gCoil_R :{gCoil_R}','Pass' ])
                    ...
                    if len(RX_PLAP)>2:
                        ...
                        for key,value in Check['CoilVals'].items():
                            if key in CoilVals:
                                if value[0]:result= 'Pass' if CoilVals[key]>=value[1] and CoilVals[key] <=value[2] else 'Fail'  
                                else:result='Pass' if  CoilVals[key]==value[1] else 'Fail'
                        ...
                        if len(Stable)>2: 
                            ...
                            res.append([f'Stabilization found at {{{Stable[2]}}}','Pass'])
                            res.append([f'Measured Average Received Power is {round(AVGRRP,3)} W for {len(PRECT)} {Check['PLA'][0]} Packets','Pass'])
                            res.append([f'Measured Average Prect power is {round(AVGPRECT,3)} W for {len(PRECT)} {Check['PLA'][0]} Packets','Pass'])
                            res.append([f'Measured PLossAverage is {round(AVGRRP-AVGPRECT,3)} W for {len(PRECT)} {Check['PLA'][0]} Packets','Pass'])
                            # res.append([f'Measured Average Received Power is {round(AVGRRP,3)} W & Average Prect power is {round(AVGPRECT,3)} W, PLossAverage is {round(AVGRRP-AVGPRECT,3)} W','Pass'])
                            if Count >1:
                                res.append([f' Check Current PRECT_AVE : {round(AVGPRECT,3)} W is within 5% of TestP1 -PRECT_AVE :{round(AveragePLoss[1][2],3)} W  ','Pass' if AVGPRECT >= AveragePLoss[1][3][0] and AVGPRECT <= AveragePLoss[1][3][1] else "Fail"])
                            if Check['PLA'][0]=="PLA_2 [0x88]":
                                ...
                                if len(result)>0: res.extend(result)
                        else:  res.append([f'PRx did not Stabilized to Negotiable Power','Inconclusive'])  
                    else:res.append([f'PRx did not sent {Check['PRx_PLAP'][0]} PKt','Inconclusive'])
                else:res.append([f'Test did not found {Check['Pkt'][0]}','Inconclusive'])
            res.append([f'Check Ploss Average form P2 {AveragePLoss[2][0]} W > Ploss Average P1 {AveragePLoss[1][0]} W','Pass' if AveragePLoss[2][0] > AveragePLoss[1][0] else 'Fail'])
            res.append([f'Check Ploss Average form P1 {AveragePLoss[1][0]} W > Ploss Average P3 {AveragePLoss[3][0]} W','Pass' if AveragePLoss[1][0] > AveragePLoss[3][0] else 'Fail'])
    ...
    def Power_Check(self,CTSCheck,Check,flows,flwID):
        ...
        if len(pkt)>2:
            res.append([f'{'PRx' if Type=='Packet' else 'TPT'} sent the {PktName} Packet at {{{pkt[2]}}}','Pass'])
                ...
                match Check_Name['comp']:
                    case 'EQL':
                        if power==Check_Name['power']:
                            res.append([f'{'PRx' if Type=='Packet' else 'TPT'}  sent the {Check_Name['Name']} with val {int(power)} W Exp :{Check_Name['power']} W','Pass'])
                        else: res.append([f'{'PRx' if Type=='Packet' else 'TPT'}  sent the {Check_Name['Name']} with val {int(power)} W Exp :{Check_Name['power']} W','Fail'])
        else:res.append([f'{'PRx' if Type=='Packet' else 'TPT'} did not sent the {PktName} datapacket.', "Inconclusive"])
        return res
    ...
    def PacketDetails(self,CTSCheck,Check,flows,flwID):
        ...
        for ExpPacket in Check['ExpectedPacket']:
            ...
            if len(ExpectedPacket_Details)>2:
                res.append([f'{'PRx' if Type=='Packet' else 'TPT'}  sent the {DataPacket} datapacket at {{{ExpectedPacket_Details[2]}}}', "Pass"]) 
                if ExpPacket[2]: 
            ...
            else:res.append([f'{'PRx' if Type=='Packet' else 'TPT'} did not sent the {DataPacket} datapacket.', "Fail"]) 
        return res
    ...
    def XID_Check(self,CTSCheck,Check,flows,flwID):
        ...
        if Check.get('EPT',False):
            ...
            EPT=self.PktMethod.GetPacketDetails(packet="End Power Transfer", value= "[EPT/rep]", limit=self.Flow_limit)
            if len(EPT)>2:
                res.append([f'PRx sent EPT/rep Packet at {{{EPT[2]}}} to proceed to Digital Ping 360 kHz','Pass'])
            else:res.append([f'PRx did not sent EPT/rep Packet','Inconclusive'])
        ...
        if SSID is not None and self.file_list[SSID]['pktType'] in'Signal strength':
            ...
            if ID is not None and self.file_list[ID]['pktType'] in 'Identification':
                ...
                if XID is not None and self.file_list[XID]['pktType'] in ["MPP_Extended_Identification","Extended Identification"]:
                    res.append([f'PRx sent the {self.file_list[XID]['pktType']} data packet at {{{XID}}} after Sequence Digital Ping - Signal Strength - Identification ','Pass'])
                    if Check.get("BitCheck",False):
                        FE_Check=self.PktMethod.GetPayloadDetails(XID,'XID_Selector')[0]['sRawData']
                        res.append([f'PRx sent the XID selector Field as : {FE_Check} ,Expected:0xFE ','Fail' if FE_Check !="0xFE" else 'Pass'])
                else: res.append([f'PRx sent the {self.file_list[XID]['pktType']} data packet after Identification Packet','Fail'])
            else:res.append([f'PRx did not send Identification Packet after SS Packet','Fail'])
        else:res.append([f'PRx did not send Signal Strength Packet','Fail'])
            ...
            while id < self.Flow_limit[1]:
                ...
                if len(PLA)>2:
                    ...
                    if RPcount %2==0:
                        ...
                        if Response is not None:
                            res.append([f'TPT sent Response:{Response[0]} for the 2nd {Check['PowerPkt']} data packet at {{{PLA[2]}}}.', 'Inconclusive' if 'ATN' not in Response[0] else 'Pass'])
                            ...
                            if len(DSR)>2:
                                ...
                                if Response is not None:
                                    if Check['Response'] in Response[0]:
                                        count+=1
                                        res.append([f'Sequence - {count}','Pass'])
                                        res.append([f'TPT sent Response : {Check['Response']} for the DSR/poll Packet at {{{DSR[2]}}}', 'Pass'])
                                        # find UUT packet
                                        UUTpkt=self.PktMethod.GetPacketDetails(packet="DSR",value="ND",limit=[Response[1],NextATN])
                                        if len(UUTpkt)>2:
                                            res.append([f'PRx  sent DSR/ND Packet at {{{UUTpkt[2]}}} for the PTx -{Check['Response']}', 'Pass'])
                                        else:res.append([f'PRx did not sent DSR/ND Packet for the PTx -{Check['Response']} at {{{Response[1]}}} until Next {Check['PowerPkt']} ATN at {{{NextATN}}} ', 'Fail'])    
                                    else: res.append([f'TPT sent Response : {Response[0]} for the DSR/poll Packet at {{{DSR[2]}}}', 'Inconclusive'])     
                                else:res.append([f'TPT did not sent Response : {Check['Response']} for the DSR/poll Packet at {{{DSR[2]}}}', 'Inconclusive'])
                            else:res.append([f'PRx did not sent DSR/POLL Packet from the ATN at {{{Response[1]}}} to Next {Check['PowerPkt']} ATN at {{{NextATN}}} ', 'Inconclusive'])
                        else:res.append([f'TPT did not sent Response for the 2nd {Check['PowerPkt']} data packet at {{{PLA[2]}}}.', 'Inconclusive'])
                    id=PLA[2]+1
                else:break
            
            if count < Check['PktsCount'] :res.append([f'Test did not found {Check['PktsCount']} Protocol Sequences of {Check['Response']} Packets','Inconclusive'])
        else: res.append([f'PRx did not Entered in to Power Transfer Phase','Inconclusive'])
        return res
    ...
    def Tdsr(self,CTSCheck,Check,flows,flwID):
        ...
        if len(PT)>2:
                ...
                if len(DSR)>2:
                    ...
                    if Response is not None:
                        if Check['Response'] in Response[0]:
                            ...
                            if len(UUTpkt)<2: res.append([f'PRx did not sent DSR Packet after the TPT {Check['Response']} at {{{Response[1]}}}, so Tdsr =-1ms', 'Fail'])  
            ...
            if FailCount >  int(0.05 * len(Timings)):
                res.append([f'Measured  Max Tdsr is {max(Timings)} mS ,Min Tdsr is {min(Timings)} mS Limit :0 < ≤ 1,000 ms ', "Fail"])
                res.append([f'More than 5% of the Intervals met the fail criteria', "Fail"])
            else:res.append([f'Measured  Max Tdsr is {max(Timings)} mS ,Min Tdsr is {min(Timings)} mS Limit :0 < Tdsr ≤ 1,000 ms ', "Pass"])
            if count < Check['PktsCount'] :res.append([f'Test did not found {Check['PktsCount']} Protocol Sequences of {Check['Response']} Packets','Inconclusive'])
        else: res.append([f'PRx did not Entered in to Power Transfer Phase','Inconclusive'])
        return res
    ...
    def FSK_Demod(self,CTSCheck,Check,flows,flwID):
        ...
        if len(MPP)>2:
            ...
            if not Flow2RestrictedBit :
                res.append([f'PRx set Restricted Bit to Zero in MPP_Extended_Identification at {{{MPP[2]}}}', 'Pass'])
                # Update Negotiable Load Power in ECAP Pkt
                EcapPkt=  "Extended_Power_Transmitter_Extended_Capabilities" if self.Certification in [ "2.0.1",  "2.1.0", "2.2.1", "2.3.0","2.0.0"] else "Power Transmitter Extended Capabilities"
                ECAP=self.PktMethod.GetPacketDetails(packet=EcapPkt, Type="Response",limit=self.Flow_limit)
                if len(ECAP)>2:
                ...
                PhaseLimit=self.FindPhase(self.Flow_limit[0],"PT")
                if PhaseLimit is not None :
                    res.append([f'PRx  Entered in to Power Transfer Phase','Pass']) 
                    # Check Stabilization
            ...
            else:res.append([f'PRx set Restricted Bit to one in MPP_Extended_Identification at {{{MPP[2]}}}', 'Inconclusive'])
        else:res.append([f'PRx did not Entered in to Power Transfer Phase','Fail']) 
        return res
        ...
        if len(Nego)>2:
            ...
            if len(ECAP)>2:
                ...
                if not Check.get('CPM',False) and NPowerCal > 15:
                    ...
                    res.append([f'TPT set Negotiable Load power to {NPower} W in ECAP Packet at {{{ECAP[2]}}} with Power limit reason:{int(Reason)} -- Before DPLoss Calibration. In SDF Guranteed Load Power was set to {GPower} W ','Pass' if Reason==10 else 'Inconclusive'])   
                    ResultFlag=True
                else:
                    res.append([f'TPT set Negotiable Load power to {NPower} W in ECAP Packet at {{{ECAP[2]}}} Expected val :{NPowerCal} W. In SDF Guranteed Load Power was set to {GPower} W ','Pass' if NPower== NPowerCal else 'Inconclusive'])   
            else: res.append([f'Test did not found ECAP data pkt', 'Inconclusive'])
                ...
                if len(Stable)>2:
                    ...
                    if ResultFlag:
                        ...
                        if len(ECAP)>2:
                            NPower=(self.PktMethod.hex_to_decimal(self.PktMethod.GetPayloadDetails(ECAP[2],'Negotiable_Load_Power')[0]['sRawData']))/10
                            Reason= self.PktMethod.hex_to_decimal(self.PktMethod.GetPayloadDetails(ECAP[2],'Power_Limit_Reason')[0]['sRawData'])
                            res.append([f'TPT set Negotiable Load power to {NPower} W in ECAP Packet at {{{ECAP[2]}}} with Power limit reason:{int(Reason)} -- After DPLoss Calibration. In SDF Guranteed Load Power was set to {GPower} W ','Pass' if Reason==0 else 'Inconclusive'])   
                        else: res.append([f'Test did not found ECAP data pkt after DPLoss Calibration', 'Inconclusive'])
        ...
        for pkt in range(self.Flow_limit[0], self.Flow_limit[1]):
            if Check['pkt'] in self.file_list[pkt]['pktType']:
                ...
                if Check['mode'] in self.file_list[pkt]['value']:
                    res.append([f"Power mode: '{pm.group(1)}' found in packet {self.file_list[pkt]['pktType']} at index {pkt} [Expected: {Check['mode']}]","Pass"])
                else:
                    res.append([f"Found Power mode of {self.file_list[pkt]['value']} in the packet {self.file_list[pkt]['pktType']} at index-{pkt} [Expected: {Check['mode']}]", "Inconclusive"])
                break
        return res
    ...
    def ValidateEye(self,CTSCheck,Check,flows,flwID):
        ...
        if values['pkts']>0:
            ...
            res.extend(results)
            # Validate
            if values['Npass_Magnitude']>0:res.append([f'Number of packets which pass the magnitude test is  : {values['Npass_Magnitude']}','Pass'])
            if values["Npass_Phase"]>0:res.append([f'Number of packets which pass the Phase test is  : {values['Npass_Phase']}','Pass'])
            if Check.get('Ntotal',False):
                
                    res.append([
                        f"Calculated Npass {values['Npass']} and Received Ntotal {values['pkts']} Expected : Npass >=Ntotal-2",
                        "Pass" if values['Npass'] >= values['pkts']-2 else "Fail"
                    ])
                    ...
                    res.append([
                        ...
                        "Pass" if values['Npass'] >= required else "Fail"
                    ])
            else: res.append([f'Number of packets which pass the magnitude and / or phase test -- Npass is  : {values['Npass']}, Expected :>={Check['Npass']}','Pass' if values['Npass'] >= Check['Npass'] else 'Fail'])
        else:res.append([f'Test did not found sufficient Packets for Measurements','Inconclusive'])
        return res
    ...
    def ParityBits(self,CTSCheck,Check,flows,flwID):
        ...
        if len(MPP)>2:
            Flow2RestrictedBit=self.MPPRestrictedBit(MPP[2])
            if Flow2RestrictedBit:res.append([f'PRx supports MPP Restricted Mode','Inconclusive'])
                
            else:
                res.append([f'PRx did not supports MPP Restricted Mode','Pass'])
                pktfound=False
                id=MPP[2]+1
                while id < self.Flow_limit[1]:
                    ...
                    if len(pkt)>2:
                        ...
                        if Response is not None:
                            if Response[0] in ["Extended_Power_Transmitter_Extended_Capabilities", "Power Transmitter Extended Capabilities"]:  res.append([f'TPT sent Response {Response[0]} for PRx GET[ECAP] request Pkt at {{{pkt[2]}}} ','Pass'])
                            else: res.append([f'TPT sent Response {Response[0]} for PRx GET[ECAP] request Pkt at {{{pkt[2]}}} ','Inconclusive'])
                        else:
                            ...
                            if PktorResId is not None:
                                res.append([f'TPT did not sent Response  for PRx GET[ECAP] request Pkt at {{{pkt[2]}}} ','Inconclusive']) 
                            else:break
                        id=pkt[2]+1
                    else:break
                if not pktfound:res.append([f'PRx did not sent GET[ECAP] request Pkt ','Inconclusive'])
                # find End Nego
                SRQ_En=self.PktMethod.GetPacketDetails(packet="SRQ [0x20] ",value="End Negotiation", limit=[id,self.Flow_limit[1]])
                if len(SRQ_En)>2: res.append([f'PRx sent SRQ/En packet during the 360 kHz Negotiation phase','Fail'])  
                else:res.append([f'PRx did not sent SRQ/En packet during the 360 kHz Negotiation phase','Pass'])                                    
        else:res.append([f'PRx did not sent MPP Extended Identification Packet','Inconclusive'])
        return res
        ...
        for pkt in Check['Pkts']:
            ...
            while id < self.Flow_limit[1]:
                ...
                if len(XCE)>2:
                    ...
                    Response=self.PktResponse(XCE[2]+1,self.Flow_limit[1])
                    if Response is None:
                        res.append([f'TPT did not sent  Response for the XCE Packet at {{{XCE[2]}}} in the Power Transfer Phase','Inconclusive']) 
                        id=Response[2]+1
                    else:
                        if 'ATN' in Response[0]:
                            if 'RESPONSE_TIME' in self.Header['TestcaseID']: res.append([f'TPT sent {Response[0]} Response for the {'First XCE Packet' if count==1 else f'First XCE Packet at {{{XCE[2]}}} after Stabilization '}','Pass'])
                            else:res.append([f'TPT sent {Response[0]} Response for the XCE Packet at {{{XCE[2]}}}','Pass'])
                            # check Dsr
                            XcePkt=True
                            DSR=self.PktMethod.GetPacketDetails(packet="DSR",value="POLL", limit=[XCE[2]+1,self.Flow_limit[1]])
                            if len(DSR)>2:
                                ...
                                if Response is not None:
                                    if pkt[0] in self.file_list[Response[1]]['pktType'] and pkt[1] in self.file_list[Response[1]]['value']:
                                        res.append([f'TPT sent {pkt[0]}_{pkt[1]} response at {{{Response[1]}}} for the DSR/POLL Packet at {{{DSR[2]}}}','Pass'])
                                        # Check Pkt from UUT
                                        ...
                                        if len(UUTpkt)>2:
                                            ...
                                            res.append([f'PRx sent {pkt[2]} at {{{UUTpkt[2]}}} within {Timing} mS from the {pkt[0]}_{pkt[1]} , Expected :<={Check['Limit']} mS','Pass' if Timing <=Check['Limit'] else 'Fail'])
                                            id=UUTpkt[2]+1
                                        else:
                                            res.append([f'PRx did not sent {pkt[2]} Packet after {pkt[0]}_{pkt[1]}','Fail'])  
                                            id=Response[1]+1
                                        break
                                    else: res.append([f'TPT sent Response as {self.file_list[Response[1]]['pktType']}_{self.file_list[Response[1]]['value']} for the DSR/POLL Packet {{{DSR[2]}}}','Inconclusive'])   
                                    break
                                else:  res.append([f'TPT did not sent  Response for the DSR/POLL Packet at {{{DSR[2]}}} in the Power Transfer Phase','Inconclusive'])     
                                id=DSR[2]+1
                                break
                            else:  res.append([f'PRx did not sent DSR/Poll after the Sequence XCE-ATN from {{{XCE[2]}}}','Inconclusive'])  
                            break  
                        else:id=Response[1]+1 
                else:break
            if not XcePkt:res.append([f'Test did not Found XCE Packet with Response : ATN ','Inconclusive'])
        return res
    ...
    def VerifyPrect_PD(self,CTSCheck,Check,flows,flwID):
        try:
            ...
            if len(Pkt1)>2:
                res.append([f"first MPP_XCE_Stabilized found at {round(Pkt1[0],3)}sec","Pass"])
                #Get First PLA
                ...
                if len(PLA1)>2:
                    ...
                    res.append([f"PLA1 packet found after first MPP_XCE_Stabilized packet at {round(PLA1[0],3)}sec with Prect={Prect1}W","Pass"])
                    #Get Second PLA
                    Pkt2 = self.PktMethod.GetPacketDetails(packet="MPP_XCE_Stabilized",limit=[PLA1[2]+1,self.Flow_limit[1]],Type="TesterMsg")
                    if len(Pkt2)>2:
                        res.append([f"Second MPP_XCE_Stabilized found at {round(Pkt2[0],3)}sec","Pass"])
                        ...
                        if len(PLA2)>2:
                            ...
                            res.append([f"PLA2 packet found after second MPP_XCE_Stabilized packet at {round(PLA2[0],3)}sec with Prect={Prect2}W","Pass"])
                            ...
                            else:res.append([f"Issue to calculate Prect value from PLA packets","Fail"])
                        else:res.append([f"PLA2 packet not found after Second MPP_XCE_Stabilized packet","Fail"])
                    else:res.append([f"Second MPP_XCE_Stabilized not found","Fail"])
                else:res.append([f"PLA1 packet not found after first MPP_XCE_Stabilized packet","Fail"])
            else:res.append([f"first MPP_XCE_Stabilized not found","Fail"])
            return res
        except Exception as e:
            print(e)
    ...
    def PD_PLA_PrectVrect(self,CTSCheck,Check,flows,flwID):
        try:
            ...
            Pkt1 = self.PktMethod.GetPacketDetails(packet="MPP_XCE_Stabilized",limit=self.Flow_limit,Type="TesterMsg")
            if len(Pkt1)>2:
                res.append([f"MPP_XCE_Stabilized found at {round(Pkt1[0],3)}sec","Pass"])
                ...
                if len(PLA1)>2:
                    ...
                    res.append([f"First PLA packet found at {round(PLA1[0],3)}sec Measured Prect_P1={round(PLA1_Prect,3)}W and V1={round(PLA1_Vrect[0],3)}V","Pass"])
                    ...
                    if len(PLA2)>2:
                        ...
                        res.append([f"PLA2 packet found at {round(PLA2[0],3)}sec Measured Prect_P2={round(PLA2_Prect,3)}W and V2={round(PLA2_Vrect[0],3)}V","Pass"])
                    ...
                    else:res.append(["PLA2 packet not found after first PLA","Fail"])
                else:res.append(["PLA1 packet not found after MPP_XCE_Stabilized","Fail"])
            else:res.append([f"MPP_XCE_Stabilized not found for the 360Khz flow","Fail"])
            return res
        except Exception as e:
            traceback.print_exc()
    ...
    def ITPT_MAX(self,CTSCheck,Check,flows,flwID):
        try:
            ...
            if len(pkt1)>2:
                res.append([f"The packet {EcapPkt} found at {round(pkt1[0],3)}sec","Pass"])
                GPower=self.BKjsonData['testBkpProjectConfiguration']['EsdfConfigurationModel']['AllESDFFields']['GuaranteedLoadPower']
                NPowerCal = math.floor(((Check['Percentage']/100) * min(15, GPower)) * 10) / 10
                NPower=(self.PktMethod.hex_to_decimal(self.PktMethod.GetPayloadDetails(pkt1[2],'Negotiable_Load_Power')[0]['sRawData']))/10
                res.append([f'TPT set Negotiable Load power to {NPower} W in ECAP Packet ,Expected val :{NPowerCal} W. In SDF Guranteed Load Power was set to {GPower} W ','Pass' if NPower== NPowerCal else 'Inconclusive'])   
            else:res.append([f"The packet {EcapPkt} not found to check the ITPT max value","Inconclusive"])
            
            pkt2 = self.PktMethod.GetPacketDetails(packet="MPP_XCE_Stabilized",limit=self.Flow_limit,Type="TesterMsg")
            if len(pkt2)>2:
                res.append([f" MPP_XCE_Stabilized found at {round(pkt2[0],3)}sec","Pass"])
                XCEVlist = []
                cnt=0
                ...
                if len(XCEVlist)>0:
                    ...
                    if all(abs(v) <= 1 for v in XCEVlist):
                        res.append([f"All 5 XCEV packet values i.e {int_values} Limit :within ±1", "Pass"])
                    else:
                        res.append([f"All 5 XCEV packet values i.e {int_values} Limit :within ±1", "Fail"])
                else:res.append[f"No XCEV packet found after stabilization","Inconclusive"]
            else:res.append([f"MPP_XCE_Stabilized not found to check the ITPT max value","Inconclusive"])
            if len(pkt1)>2 and len(pkt2)>2:
                ...
                else: res.append([f"No MPLA_AC_DC_Current packets found between {round(pkt1[0],3)}sec to {round(pkt2[0],3)}sec","Inconclusive"])
                ...
                else:res.append([f"PLA packet not found after stabilization","Fail"])
            return res
        except Exception as e:
            print(e)
    ...
    def ValidateNego(self,CTSCheck,Check,flows,flwID):
        ...
        if len(pings_360)>=7:
            ...
            for ping in pings_360:
                ...
                if len(pkt1)>2:
                    res.append([f'PRx sent Configuration pkt at {{{pkt1[2]}}} ','Pass'])
                    ...
                    else:res.append([f'PRx did not sent Configuration in the Sequence :{count}','Inconclusive'])
                else:res.append([f'PRx did not sent SRQ_End Negotiation in the Sequence :{count}','Inconclusive'])
                count+=1
        else:res.append([f'TPT did not sent Enough Pings for measurements','Inconclusive'])
        return res
    ...
    def RestrictedMode(self,CTSCheck,Check,flows,flwID):
        ...
        if len(MPP)>2:
            ...
            if Flow2RestrictedBit:
                ...
                End_Power=self.PktMethod.GetPacketDetails(packet="End Power Transfer", value= "[EPT/rst]",limit=[MPP[2]+1,self.Flow_limit[1]])
                if len(End_Power)>2:
                   res.append([f'PRx sent End Power Tranfer Packet _RST to Switch from Restricted Mode to MPP-Full Mode','Pass'])
                else:
                    Renego=self.PktMethod.GetPacketDetails(packet="Renegotiate", limit=[MPP[2]+1,self.Flow_limit[1]])
                    if len(Renego)<2:res.append([f'PRx did not sent Renegotiate Packet to Switch from Restricted Mode to MPP-Full Mode','Fail'])
                    else:
                        ...
                        else:res.append([f'PRx sent only Control Error and 8 bit Received Power Packets Before Renegotiation at {{{Renego[2]}}}','Pass'])
                        ...
                        else:res.append([f'PRx sent only Extended Control Error and PLA Packets after Renegotiation at {{{Renego[2]}}}','Pass'])
            else:res.append([f'PRx did not set the Restricted Bit to 1 in MPP_Extended_Identification PKT at {{{MPP[2]}}} in to 360 Flow','Fail'])  
        else:res.append([f'PRx did not sent MPP-Extended Identification Packet in 360 Flow','Inconclusive'])   
        return res
    ...
    def ProfileActivation(self,CTSCheck,Check,flows,flwID):
        ...
        if len(MPP)>2:
            ...
            if Flow1RestrictedBit:
                res1.append([f'PRx supports Restricted Mode at DP 128kHz','Pass'])
                if Check['flow']==2:
                    ...
                    if len(CFG)>2:
                        ...
                        else:res2.append([f'PRx sent only Control Error and 8 bit Received Power Packets after CFG at {{{CFG[2]}}}','Pass'])
                    else:res2.append([f'PRx did not sent CFG Packet','Inconclusive'])
            else:
                res1.append([f'PRx did not supports Restricted Mode at DP 128kHz','Pass'])
                ...
                if Check['flow']==1:
                    for pkt in Check['Pkts']:
                        ...
                        if len(Pac)>2:
                            ...
                            if Response is not None:res1.append([f'TPT sent Response {Response[0]} for the {pkt[0]} {"" if pkt[1] is None else pkt[1]} Packet at {{{Pac[2]}}}','Pass'])
                            else:res1.append([f'TPT did not sent Response for the {pkt[0]} {"" if pkt[1] is None else pkt[1]} Packet at {{{Pac[2]}}}','Fail'])  
                            id=Pac[2]+1 
                        else:res1.append([f'PRx did not sent {pkt[0]} {"" if pkt[1] is None else pkt[1]} Packet' ,'Inconclusive'])

                    # Check EPT pkt
                    End_Power=self.PktMethod.GetPacketDetails(packet="End Power Transfer", value= "[EPT/rep]",limit=[id,flow1[1]])
                    if len(End_Power)>2:
                        res1.append([f'PRx sent End Power Transfer Re-Ping Packet at {{{End_Power[2]}}} to Enter in to 360 Phase','Pass'])
                    else:res1.append([f'PRx did not sent End Power Transfer Re-Ping Packet in 128 Flow','Inconclusive'])
                else:
                    ...
                    if len(MPP)>2:
                        ...
                        if not RestrictedBit:
                            res2.append([f'PRx did not supports Restricted Mode at DP 360kHz','Pass'])
                            # check Nego Pkts
                            iid=MPP[2]+1
                            while iid < flow2[1]:
                                if self.file_list[iid]['description']=="Nego" and not self.file_list[iid]['isTesterPkt']:
                                    ...
                                    # if self.file_list[iid]['pktType'] in Check['NegPkts']:
                                        res2.append([f'PRx sent {self.file_list[iid]['pktType']} Packet at {{{iid}}} in Nego Flow','Pass'])
                                    else:
                                        res2.append([f'PRx sent {self.file_list[iid]['pktType']} Packet at {{{iid}}} in Nego Flow which is not in List :{Check['NegPkts']}','Fail'])
                                if self.file_list[iid]['description']=="PT":break 
                                iid+=1
                        else: res2.append([f'PRx set the Restricted Bit to 1 in MPP_Extended_Identification PKT at {{{MPP[2]}}} in to 360 Phase','Fail'])
                    else:res2.append([f'PRx did not sent MPP-Extended Identification Packet in 360 Flow','Inconclusive'])   
    ...
    def MPLA_PrectCheck(self,CTSCheck,Check,flows,flwID):
        ...
        # Check Stabilization
        Stable=self.PktMethod.GetPacketDetails(packet="Stabilized",Type="TesterMsg",limit=self.Flow_limit)
        res.append([f'PRx {'did not Stabilized to Negotiable Load Power'  if len(Stable)<2 else ''} Stabilized at {{{Stable[2]}}}','Pass' if len(Stable)>2 else 'Inconclusive' ])
        ...
        while id < self.Flow_limit[1]:
            ...
            if Response is not None :
                if'NAK' in Response[0]:
                    res.append([f'TPT sent {Response[0]} for PLA packet at {{{FPLA[2]}}}','Inconclusive'])
            else:res.append([f'TPT did not sent response for PLA packet at {{{FPLA[2]}}}','Inconclusive'])
            ...
            if len(FPLA)>2:
                ...
                if len(SPLA)>2:
                    ...
                    if SPrect >7.5:
                        ...
                        if not result:
                            res.append([f' Measured PRECT_delta for PLA between {{{FPLA[2]}}} and PLA at {{{SPLA[2]}}} is {delta} W ,Limit :{Limit}','Fail'])
        ...
        if Timings:
            ...
            res.append([f'Measured Max PLA from {max_item[0]} Sec to {max_item[1]} Sec at {{{max_item[2]}}} is {max_item[-1]} mS --- for {len(Timings)} {Check['Pkt'][0]} Packets, Limit: <=2050 mS', 'Fail' if  max_item[-1] > 2050 else 'Pass'])
            res.append([f'Measured Min PLA from {min_item[0]} Sec to {min_item[1]} Sec at {{{min_item[2]}}} is {min_item[-1]} mS --- for {len(Timings)} {Check['Pkt'][0]} Packets, Limit: <=2050 mS', 'Fail' if  min_item[-1] > 2050 else 'Pass'])
        else:
            res.append([f'Test did not found any MPLA Packets','Fail'])

        if PrectDelta:
            min_item = min(PrectDelta, key=lambda x: x[-1])
            max_item = max(PrectDelta, key=lambda x: x[-1])
            res.append([f'Measured Max PRECT_delta from {max_item[0]} Sec to {max_item[1]} Sec at {{{max_item[2]}}} is {max_item[-1]} W','Pass'])
            res.append([f'Measured Min PRECT_delta from {min_item[0]} Sec to {min_item[1]} Sec at {{{min_item[2]}}} is {min_item[-1]} W', 'Pass'])
        ...
        if PTPhaseLimit is not None:
            ...
            if len(DSR)>2:
                ...
                if Response is not None and 'Coex' in cl_reason:
                    res.append([f'TPT sent {Response[0]} Response with reason:{cl_reason} for the DSR/poll Packet at {{{DSR[2]}}}', 'Pass'])
                    ...
                    if len(Cloak)>2:
                        ...
                        if Response is not None:
                           res.append([f'TPT sent {Response[0]} Response for the Cloak Packet at {{{Cloak[2]}}}', 'Pass' if 'ACK' in Response[0] else 'Inconclusive']) 
                           id=Response[1]+1
                           # find TCloakDelay
                           ...
                           while id < self.Flow_limit[1]:
                                ...
                                if len(Attach)>2:
                                    if count==2:
                                        ...
                                        res.append([f'TPT delayed 3rd Cloak Ping by {Timeduration} mS , Exp :{DelayTime-1} mS','Pass' if Timeduration > DelayTime-10 else 'Inconclusive'])
                                        # find Cloak pkt 
                                        Cloak=self.PktMethod.GetPacketDetails(packet="Cloak", limit=[Attach[2]+1,self.Flow_limit[1]])
                                        if len(Cloak)>2:
                                            res.append([f'PRx sent Cloak packet at {{{Cloak[2]}}} for the Cloak ping : {count+1} ','Fail'])
                                        else:res.append([f'PRx did not sent Cloak packet for the Cloak ping : {count+1} ','Pass'])
                                        ...
                                        if len(Ping)>2:
                                            ...
                                            if len(SS)>2:res.append([f'PRx sent Signal Strength Packet at {{{SS[2]}}} for the Next Digital ping at {{{Ping[2]}}}','Pass'])
                                            else:res.append([f'PRx did not sent Signal Strength Packet for the ping at {{{Ping[2]}}}','Fail'])
                                            
                                        else:res.append([f'TPT did not Initiated next ping after Cloak Exit','Inconclusive'])
                                    ...
                                    if len(Detach)>2:
                                        ...
                                        if len(Cloak)>2:
                                            res.append([f'PRx sent Cloak packet at {{{Cloak[2]}}} for the Cloak ping : {count} ','Pass'])
                                        else:res.append([f'PRx did not sent Cloak packet for the Cloak ping : {count} ','Fail'])
                                        id=Detach[2]+1
                                    else:break 
                                else:break    

                        else: res.append([f'TPT did not sent any Response for Cloak Packet at {{{Cloak[2]}}}','Inconclusive']) 
                    else:res.append([f'PRx did not sent Cloak pkt after the FSK CLOAK at {{{Response[1]}}}','Inconclusive']) 
                else:res.append([f'TPT did not sent any Response for DSR/Poll at {{{DSR[2]}}}','Inconclusive'])
            else:res.append([f'PRx did not Initiated DSR/Poll Packet','Inconclusive'])
        else:res.append([f'PRx did not Entered to Power Transfer Phase','Inconclusive'])
    ...
    def MagneticPresenceCheck(self,CTSCheck,Check,flows,flwID):
        ...
        if flwID==2: self.Flow_limit = flows[flwID]['Limit']
            ...
            else: return [f'Test Data not Found after removing the Magnetic Cover','Inconclusive']
        ...
        if len(PT)>2:
            ...
            while id <self.Flow_limit[1]:
                ...
                id+=1
            if count>=50:
                res.append([f'PRx sent {count} Packets in Power Transfer Phase','Pass'])

                if len(CE[Check['CE']])>0:res.append([f'PRx sent {len(CE[Check["CE"]])} {Check["CE"]} Packets in Power Transfer Phase','Pass'])
                else:res.append([f'PRx did not sent any {Check["CE"]} Packets in Power Transfer Phase','Fail'])

                if len(CE[Check['CE2']])>0:
                    res.append([f'PRx sent {len(CE[Check["CE2"]])} {Check["CE2"]} Packets in Power Transfer Phase','Fail'])
                else:res.append([f'PRx did not sent any {Check["CE2"]} Packets in Power Transfer Phase','Pass'])    

            else:res.append([f'PRx did not sent only {count} Packets in Power Transfer Phase , Exp :50','Inconclusive'])
    ...
    def EyeTest(self,Check):
        ...
        for pktcount in Eyeresults:
            values['pkts']+=1    
            # pkts+=1
            res.append([f'PRx sent {Eyeresults[pktcount]['Packet']} at { Eyeresults[pktcount]['Timestamp']} Sec', 'Pass'])
            dum1=[]
            ...
            for key in Check['Magnitude']:
                ...
                if  Eyeresults[pktcount]['Magnitude Results'][key]['Value'] is not None:
                    ...
                    if key =="Amplitude" and Check.get('Modulation',False)and not result:
                        ...
                        dum1.append([f'Measured Magnitude : Modulation Index Measurement is { Eyeresults[pktcount]['Magnitude Results']["Modulation Index"]['Value']} Limit:{Eyeresults[pktcount]['Magnitude Results']["Modulation Index"]['Limit']} ','Pass' if result else 'Fail'])
                    else:dum1.append([f'Measured Magnitude :{key} Measurement is { Eyeresults[pktcount]['Magnitude Results'][key]['Value']} Limit:{Eyeresults[pktcount]['Magnitude Results'][key]['Limit']} ','Pass' if result else 'Fail'])

                    if not result:MagnitudeResult=False
                else:
                    dum1.append([f'Measured Magnitude :{key} is None','Inconclusive'])
            ...
            if not MagnitudeResult:
                ...
                for key in Check['Phase']:
                    ...
                    if  Eyeresults[pktcount]['Phase Results'][key]['Value'] is not None:
                        ...
                        dum2.append([f'Measured Phase :{key} Measurement is {Eyeresults[pktcount]['Phase Results'][key]['Value']} Limit:{Eyeresults[pktcount]['Phase Results'][key]['Limit']} ','Pass' if result else 'Fail'])
                    else: 
                        dum2.append([f'Measured Phase :{key} is None','Inconclusive'])
                        PhaseResult=False    
                ...
                for pkt in pkts:
                    if pkt[0] in self.file_list[id]['pktType'] and True if pkt[1] is None else pkt[1] in self.file_list[id]['value']:
                        ...
                        if Response:
                            ...
                            Response=self.PktResponse(id+1,self.Flow_limit[1])
                            if Response is not None:
                                res.append([f'TPT sent {Response[0]} at {{{Response[1]}}} for PRx - {PktName} Packet at {{{id}}}','Pass' if pkt[2] in Response[0] else 'Inconclusive'])
                            else:res.append([f'TPT did not sent Response for the Pkt {PktName} at {{{id}}}','Inconclusive'])
                        else:res.append([f'PRx sent the  {PktName} at {{{id}}}','Pass'])
                        break
                if not pktfound: res.append([f' PRx sent the Pkt {self.file_list[id]['pktType']} {self.file_list[id]['value']} at {{{id}}} which is not a Nego Phase Pkt','Inconclusive' if Inconclusive else 'Fail'])      
            id+=1
        return res
        ...
        while Pd_id < len(PayLoads):
            ...
            for payload in self.PktMethod.GetGeneralPayloadDetails(name=PayLoads[Pd_id].get("Name"),index=Index,Byte=PayLoads[Pd_id].get("Byte"),Bit=PayLoads[Pd_id].get("Bit")):
                ...
                if raw_data:
                    ...
                    Check=True
            if not Check:res.append([f'{'PRx'if Receiver else 'PTx'} did not sent Expected {PayLoads[Pd_id].get("Name")} in the {PacketName} Data Packet  at {{{Index}}}, Exp:{PayLoads[Pd_id].get("Exp")} , Comp :{PayLoads[Pd_id].get("comp")}.','Fail'])   
            Pd_id+=1
        return res
    ...
    def Check_CE_PLA(self,start,end,CE,PLA):
        ...
        while id < end:
            if self.file_list[start]['pktType']==CE  or self.file_list[start]['pktType']==PLA:
                res.append([f'PRx sent {self.file_list[start]['pktType']} at {{{start}}}','Fail'])
            id+=1
        return res
        ...
        for pkt in pkts:
            ...
            while id < limit[1]:
                if pkt[0] in self.file_list[id]['pktType'] and True if pkt[1] is None else pkt[1] in self.file_list[id]['value']:
                    ...
                    Response=self.PktResponse(id+1,self.Flow_limit[1])
                    if Response is not None:
                        res.append([f"{ pkt[0]} {"" if pkt[1] is None else pkt[1]} is found at {{{id}}} with Response as {Response[0]}","Pass" if pkt[2] in Response[0] else 'Fail'])
                    else:res.append([f"{ pkt[0]} {"" if pkt[1] is None else pkt[1]} is found at {{{id}}} with No Response ","Fail"])
                id+=1
        return res
    ...
    def PLA2_Fast(self,Pkt,limit,TimeLimit=[],Percentage=None,Target=None):
        ...
        for Time in TimingsId:
            ...
            if TimeLimit[0]=="GTE":
                if Time[0] < TimeLimit[1]:
                    if Percentage is None:res.append([f'Measured Tpla_fast from {Pkt} at id{Time[1]} to {Pkt} at id{Time[2]}is {Time[0]} mS Limit : >={TimeLimit[1] } mS', "Fail"])
                    FailCount+=1
            if TimeLimit[0]=="LTE":
                if Time[0] > TimeLimit[1]:
                    if Percentage is None:res.append([f'Measured Tpla_fast from {Pkt} at id{Time[1]} to {Pkt} at id{Time[2]}is {Time[0]} mS Limit : <={TimeLimit[1] } mS', "Fail"])
        ...
        if Percentage is not None:
            if FailCount >  int((Percentage/100) * len(Timings)):
                res.append([f'Measured  Max Tpla_fast is {max(Timings)} mS ,Min Tpla_fast is {min(Timings)} mS  for {len(Timings)} Packets ,Limit :{TimeLimit[0]} {TimeLimit[1] } mS', "Fail"])
                res.append([f'More than {Percentage}% of the Intervals met the fail criteria', "Fail"])
            else:res.append([f'Measured  Max Tpla_fast is {max(Timings)} mS ,Min Tpla_fast is {min(Timings)} mS for {len(Timings)} Packets ,Limit :{TimeLimit[0]} {TimeLimit[1] } mS', "Pass"])
        else: res.append([f'Measured  Max Tpla_fast is {max(Timings)} mS ,Min Tpla_fast is {min(Timings)} mS for {len(Timings)} Packets ,Limit : {TimeLimit[0]} {TimeLimit[1] } mS', "Pass"])
    ...
    def PLA_MSR(self,NegotiablePower,Check,limit,NegotiablePercentage=10):
        ...
        if len(Stable)>2:                                           
            ...
            if len(FirstPLA)>2:
                ...
                res.append([f"Measured Prect_Power value in {Check['Pkt'][0]} pkt after Stabilization is {Power} W  at {{{FirstPLA[2]}}}, Expected Range :[{round((NegotiablePower-(NegotiablePercentage/100)*NegotiablePower),2)},{round((NegotiablePower+(NegotiablePercentage/100)*NegotiablePower),2)}]", "Inconclusive" if Power > (NegotiablePower+(NegotiablePercentage/100)*NegotiablePower) or Power < (NegotiablePower-(NegotiablePercentage/100)*NegotiablePower) else 'Pass'])
                id=FirstPLA[2]+1
                Count=0
                while id <limit[1]:
                ...
                res.append([f"PRx did not sent the {Check['Pkt'][0]} after Stabilization",'Inconclusive'])
        else:res.append([f'PRx did not stabilized to Negotiable Load Power','Inconclusive'])
            ...
            if len(PowerLevls[Level])>0:
                ...
                for Power in PowerLevls[Level]:
                    ...
                    if PFO < Check['PFORange'][Level]['2'][0] or PFO>=Check['PFORange'][Level]['2'][1]:
                        count+=1
                        if count >1:
                            results.append([f'Calculated PFO Value is {PFO} mW at {{{Power[0]}}}  is in  out of Range Expected range :{ Check['PFORange'][Level]['2']},Power level : {PL}','Fail'])
               
                results.append([f'Calculated Max PFO Value is {MaxPFO} mW and Min PFO Value is {MinPFO} mW --- for the {pkts} {Check['Pkt'][0]} Packets, Expected range :{ Check['PFORange'][Level]['2']} , Power level : {PL}' ,'Pass'])
                ...
                results.append([f'Calculated Average PFO value for the {pkts} {Check['Pkt'][0]} Packets is {AveragePFO} mW , Expected range :{ Check['PFORange'][Level]['1']},Power level : {PL}','Fail' if AveragePFO < Check['PFORange'][Level]['1'][0] or AveragePFO >=Check['PFORange'][Level]['1'][1] else 'pass'])
        return results 
        ...
        if pt_pass and cl_pass:
            ...
            for clk in range(self.Flow_limit[0], self.Flow_limit[1]):
                if 'Cloak' in self.file_list[clk]['pktType'] and not self.file_list[clk]['isTesterPkt']:
                    cloak_found = True
                    res.append([f"Cloak by Prx was initiated at index-{self.file_list[clk]['rowIndex']}", "Pass"])
                    reason = self.PktMethod.GetPayloadDetails(clk, 'Reason')[0]['sDescription']
                    if Check['reason'] in reason:
                        res.append([f"Cloak_Reason: {Check['reason']} was observed", "Pass"])
                    else:
                        res.append([f"Cloak_Reason: {Check['reason']} was not observed", "Fail"])
                    clk += 1
                ...
                if 'Report' in self.file_list[rep]['pktType'] and not self.file_list[rep]['isTesterPkt']:
                    ...
                    if 'PRx Identification' in prx_id:
                        ...
                        if self.file_list[response_ack]['pktType'] == 'ACK':           
                            res.append([f"Report packet with PRx Identification found at index-{self.file_list[rep]['rowIndex']}", "Pass"])
                            res.append([f"TPT send ACK response for the Report packet at index-{self.file_list[response_ack]['rowIndex']}", "Pass"])
                            ...
                            if prx_id and report_ID:
                                ...
                                for ptx_xid in range(response_ack+1, clk_limit[1]):
                                    if 'Get Request' in self.file_list[ptx_xid]['pktType'] and not self.file_list[ptx_xid]['isTesterPkt'] and 'PTx Extended Identification' in self.file_list[ptx_xid]['value']:
                                        check_ptx_xid = True
                                        res.append([f"GET[PTx XID] request was found at index-{self.file_list[ptx_xid]['rowIndex']}", "Pass"])
                                        #response
                                        ...
                                        if self.file_list[response_xid]['pktType'] == EPTI and self.file_list[response_xid]['isTesterPkt']: 
                                            ...
                                            res.append([f"TPT responded with XID packet for the request GET[PTx XID] index-{self.file_list[response_xid]['rowIndex']}", "Pass"])
                                        break
                            if not check_ptx_xid:
                                res.append([f"GET[PTx XID] request was not found at Index- [{response_ack+1, clk_limit[1]}]", "Fail"])
                            if not check_response_xid:
                                res.append([f"TPT not responded with XID packet for the request GET[PTx XID", "Fail"])
                            ...
                            if check_ptx_xid and response_xid != 0:
                                ...
                                for pt in range(response_xid+1, clk_limit[1]):
                                    if ('PT' in self.file_list[pt]['description'] and ('Extended Control Error' in self.file_list[pt]['pktType'] or 'Power Loss Accounting' in self.file_list[pt]['pktType'])):
                                        PT_phase_check = True
                                        res.append([f"Sequence entered into PT phase again, Index-[{pt, clk_limit[1]}]", "Pass"])
                                        break
                                if not PT_phase_check:
                                    res.append([f"Sequence not enetered into the PT phase after the cloak exit. Index- [{response_xid+1, clk_limit[1]}]", "Fail"])                     
                    break
            if not report_ID:
                res.append([f"Report[Prx ID] packet was not observed at the cloak phase index- {clk_limit}", "Fail"])
            
        elif not pt_pass:
            res.append([f"The packet sequence doesn't entered PT phase", "Inconclusive"])
        else: res.append([f"The packet sequence doesn't entered Cloak phase", "Inconclusive"])
        return res
        ...
        if pt_pass and cl_pass:
            ...
            for clk in range(self.Flow_limit[0], self.Flow_limit[1]):
                if 'Cloak' in self.file_list[clk]['pktType'] and not self.file_list[clk]['isTesterPkt']:
                    ...
                    reason = self.PktMethod.GetPayloadDetails(clk, 'Reason')[0]['sDescription']
                    if Check['reason'] in reason:
                        res.append([f"Cloak_Reason: {reason} was observed", "Pass"])
                    else:
                        res.append([f"Cloak_Reason: {reason} was not observed", "Fail"])
                    clk += 1
                    #set the new_limit for cloak check
                    clk_limit = []
                    start = 0
                    ...
                    if len(clk_limit) == 2:
                        ...
                        if len(assertions_details) >= 3:
                            exp_pings = True
                    else: res.append([f"Cloak phase expected pings are not observed", "Inconclusive"])
                    break
            if exp_pings:                       
                value = []
                attach_time = []
                ...
                for i, val in enumerate(result_ms):
                    if not (limit_range[0] <= val <= limit_range[1]):
                        ...
                        res.append([f"Measured t_cloak : {val} ms was not within the range at [Start index: {start_idx}, Stop index: {stop_idx}] Expected Range:{limit_range}", "Fail"])
                        t_clk = False
                ...
                if all(t_cloak_check):
                    res.append([f"All t_cloak measured are proper or within the range, Expected range: Expected Range:{limit_range}", "Pass"])
                ...
                if Check['cloak_cycle'] is not None:
                    if len(check_clk_list) >= Check['cloak_cycle']:
                        res.append([f"{Check['cloak_cycle']}x ASK cloak packets have recieved", "Pass"])
                    else: res.append([f"{Check['cloak_cycle']}x ASK cloak packets have recieved", "Fail"])
                ...
                if Check['t_wake']:
                    ...
                    if len(check_clk_list) >= 3:
                        for s, e in zip(assertions_details,check_clk_list):
                            ...
                            print(t_wake)
                            if (19 <= t_wake <= 64):
                                res.append([f"Cloak_{cloak_count_1} - Measured t_wake: {t_wake}ms which is  within the range. Expected : 19ms <= t_wake <= 64ms", "Pass"])
                            else: res.append([f"Cloak_{cloak_count_1} - Measured t_wake: {t_wake}ms  which is not within the range. Expected : 19ms <= t_wake <= 64ms", "Fail"])
                            cloak_count_1 += 1
                ...
                for res_1 in range(0, len(check_clk_list)-1):
                    ...
                    if Check['reason'] in reason:
                        ...
                        reason_cloak_1.append(reason_cloak_check)
                        # print(reason_response)
                        res.append([f"Cloak_Reason: {Check['reason']} was not observed at packet index: {check_clk_list[res_1]}", "Fail"])
                if all(reason_cloak_1):
                    res.append([f"All observed cloak reason are {Check['reason']}", "Pass"])
                ...
                if Check['report_check']:
                    for rep in range(clk_limit[0], clk_limit[1]):
                        ...
                        if 'Report' in self.file_list[rep]['pktType'] and not self.file_list[rep]['isTesterPkt']:
                            ...
                            if 'PRx Identification' in prx_id:
                                if report_count == 1:
                                    res.append([f"1st Report PRx ID packet was observed at index-{self.file_list[rep]['rowIndex']}", "Pass"])
                                else:
                                    res.append([f"2nd Report PRx ID packet was observed at index-{self.file_list[rep]['rowIndex']}", "Pass"])
                                report_ID = True
                                ...
                                if report_count == 1 and response_ack_1 == 0:
                                    res.append([f"TPT muted the reply for the 1st PRx ID report packet", "Pass"])
                                elif report_count == 1 and response_ack_1 != 0:
                                    res.append([f"TPT not muted the reply for the 1st PRx ID report packet", "Fail"])
                                elif report_count == 2 and response_ack_1 > 0 and self.file_list[response_ack_1]['pktType'] == 'ACK':
                                    res.append([f"TPT reply with ACK response for the PRx ID report packet. Index - {response_ack_1}", "Pass"])
                                elif report_count == 2 and response_ack_1 == 0:
                                    res.append([f"TPT not reply with ACK response for the PRx ID report packet", "Fail"])
                            report_count += 1
                    if not all(reportid) and reportid <= 1:
                        res.append([f"Expected retry of Report PRx ID packet was not observed. Index - {clk_limit}", "Inconclusive"])
                    ...
                    if len(t_response) == 2 and all(reportid):
                        ...
                        if t_response_timeout >= 15:
                            res.append([f"Observed t_response : {t_response_timeout}ms, when PTx did not respond to the 1st report packet, Limit = >= 15ms", "Pass"])
                        else: res.append([f"Observed t_response : {t_response_timeout}ms, when PTx did not respond to the 1st report packet, Limit = >= 15ms", "Fail"])     
                
            else: res.append(["Expected count of cloak pings were not initiated", "Inconclusive"])
        ...
        if pt_pass:   
            ...
            for xce in range(self.Flow_limit[0], start):
                if 'Extended Control Error' in self.file_list[xce]['pktType'] and not self.file_list[xce]['isTesterPkt']:
                    ...
                    if response_atn != 0 and self.file_list[response_atn]['pktType'] == 'ATN':
                        print(response_atn)
                        res.append([f"XCE packet with ATN response observed at index -[{response_atn}]", "Pass"])
            ...
            for dsr in range(response_atn, start):
                if 'DSR' in self.file_list[dsr]['pktType'] and 'POLL' in self.file_list[dsr]['value'] and not self.file_list[dsr]['isTesterPkt']:
                    ...
                    res.append([f"DSR packet was found at index -[{dsr}]", "Pass"])
                    ...
                    for clk in range(dsr, start):
                        if 'Cloak' in self.file_list[clk]['pktType'] and self.file_list[clk]['isTesterPkt']:
                            ...
                            if Check['reason'] in clk_reason:
                                res.append([f"Cloak by TPT was initiated at index -[{clk}]", "Pass"])
                                res.append([f"Cloak_Reason: {clk_reason} was observed", "Pass"])
                            ...
                            if self.file_list[prx_cloak]['pktType'] == 'Cloak' and not self.file_list[prx_cloak]['isTesterPkt']:
                                ...
                                res.append([f"Cloak by Prx was observed at index -[{prx_cloak}]", "Pass"])
                                ...
                                if self.file_list[cloak_response]['pktType'] == 'ACK' and self.file_list[cloak_response]['isTesterPkt']: 
                                    ...
                                    res.append([f"ACK response was observed for PRx cloak packet at index -[{cloak_response}]", "Pass"])
                            break
                    break
            ...
            if ptx_cloak_check and prx_cloak_check and len(t_ptx_cloak) == 2 and Check['t_ptx_cloak']:
                ...
                if t_ptx_cloak_value <= Check['t_ptxcloak']:
                    res.append([f"Measured t_ptx_cloak: {t_ptx_cloak_value}ms, Limit: t_ptx_cloak <= {Check['t_ptxcloak']}ms", "Pass"])
                else: res.append([f"Measured t_ptx_cloak: {t_ptx_cloak_value}ms, Limit: t_ptx_cloak <= {Check['t_ptxcloak']}ms", "Fail"])
            
            if not response_atn_check:
                res.append([f"ATN response was not observed for any XCE packet", "Inconclusive"])
            if not response_dsr_check:
                res.append([f"DSR packet response was not observed", "Inconclusive"])
            if not ptx_cloak_check:
                res.append([f"Ptx cloak packet was not observed", "Inconclusive"])
            if not prx_cloak_check:
                res.append([f"Prx cloak packet was not observed", "Inconclusive"])
            if not cloak_response_check:
                res.append([f"ACK response was not observed for the PRx cloak packet", "Inconclusive"])
            ...
            if Check['ptx_cloak'] and cloak_response_check and prx_cloak_check:
                ...
                for cl in range(new_limit[0], new_limit[1]-1):
                    if 'Cloak' in self.file_list[cl]['pktType'] and not self.file_list[cl]['isTesterPkt']:
                        clk_list.append(cl)
                        # res.append([f"Cloak-{cloak_count} packet found at {round(self.file_list[cl]['startTime'], 3)} sec", "Pass"])
                        #check response
                ...
                if len(clk_list) >= 3 and len(clk_list_res) == len(clk_list):
                    res.append([f"{cloak_count}x cloak cycle was observed. Minimum 3x cloak cycle is expected", "Pass"])
                elif len(clk_list) >= 3 and len(clk_list_res) != len(clk_list):
                    res.append([f"Cloak response missing for the cloak packet", "Inconclusive"])
                else: res.append([f"{cloak_count}x cloak cycle was observed. Minimum 3x cloak cycle is expected", "Fail"])
                ...
                if cloak_with_atn:
                    report_ID = False
                    response_ack = 0
                    res.append([f"Cloak with ATN response was observed at index -[{clk_atn}]", "Pass"])
                    #check report ID
                    for rep in range(clk_atn , end_1):
                        if 'Report' in self.file_list[rep]['pktType'] and not self.file_list[rep]['isTesterPkt']:
                            ...
                            if 'PRx Identification' in prx_id:
                                ...
                                if self.file_list[response_ack]['pktType'] == 'ACK':           
                                    res.append([f"Report packet with PRx Identification found at index -[{rep}]", "Pass"])
                                    res.append([f"TPT send ACK response for the Report packet at index -[{response_ack}]", "Pass"])
                    #Get[Ptx XID] packet and TPT respond with XID
                    check_ptx_xid = False
                    check_response_xid = False
                    response_xid = 0
                    for ptx_xid in range(response_ack+1, end_1):
                        if 'Get Request' in self.file_list[ptx_xid]['pktType'] and not self.file_list[ptx_xid]['isTesterPkt'] and 'PTx Extended Identification' in self.file_list[ptx_xid]['value']:
                            check_ptx_xid = True
                            res.append([f"GET[PTx XID] request was found at index -[{ptx_xid}]", "Pass"])
                            #response
                            ...
                            if self.file_list[response_xid]['pktType'] == EPTI and self.file_list[response_xid]['isTesterPkt']: 
                                check_response_xid = True
                                res.append([f"TPT responded with PTx_XID packet for the request GET[PTx XID] at index -[{response_xid}]", "Pass"])
                            break
                    if not check_ptx_xid:
                        res.append([f"GET[PTx XID] request was not found at Index- [{response_ack+1, self.Flow_limit[1]}]", "Fail"])
                    if not check_response_xid:
                        res.append([f"TPT not responded with PTx_XID packet for the request GET[PTx XID]", "Fail"])
                    ...
                    if check_ptx_xid and response_xid != 0:
                        ...
                        for pt_phase in range(response_xid, self.Flow_limit[1]):
                            if ('PT' in self.file_list[pt_phase]['description'] and ('Extended Control Error' in self.file_list[pt_phase]['pktType'] or 'Power Loss Accounting' in self.file_list[pt_phase]['pktType'])):
                                PT_phase_check = True
                                res.append([f"Sequence entered into PT phase again, Index-[{pt_phase, self.Flow_limit[1]}]", "Pass"])
                                break
                        if not PT_phase_check:
                            res.append([f"Sequence not enetered into the PT phase after the cloak exit. Index- [{response_xid+1, self.Flow_limit[1]}]", "Fail"])   
                                      
                else: res.append([f"Cloak with ATN response was not observed", "Inconclusive"])
    ...
    def ping_retry(self,CTSCheck,Check,flows,flwID):
        ...
        if pt_pass and cl_pass:
            ...
            for det_pg in range(self.Flow_limit[0], self.Flow_limit[1]):
                if 'SRQ [0x20] ' in self.file_list[det_pg]['pktType'] and 'Cloak Detect Ping' in self.file_list[det_pg]['value']:
                    ...
                    if match:
                        ...
                        res.append([f"Cloak Detect Ping with value {value} ms was observed at Index-{det_pg}", "Pass"])
                    if value != 0 or value != 0.0:
                        cloak_detect_value = True
                        res.append([f"SRQ-Detect ping packet was observed at index-{self.file_list[det_pg]['rowIndex']}", "Pass"])
                    ...
                    if response_detect != 0 and self.file_list[response_detect]['pktType'] == 'ACK':
                        res.append([f"ACK response was observed for the SRQ-Detect ping packet at index-{self.file_list[response_detect]['rowIndex']}", "Pass"])
                    else: res.append([f"ACK response was not observed for the SRQ-Detect ping packet", "Fail"])
                    break
            if cloak_detect_value:
                ...
                print(XCEV)
                if XCEV:
                    res.append([f"Stable was observed for the XCEV packets with {xce_values} before the MPP_XCE_Stabilized. Expected: XCEV +- 3", "Pass"])   
                else: res.append([f"Stable was not observed for the XCEV packet with value in {xce_values} before the MPP_XCE_Stabilized. Expected: XCEV +- 3", "Fail"])
                ...
                for xce in range(new_limit[0], new_limit[1]):
                    if 'Extended Control Error' in self.file_list[xce]['pktType'] and not self.file_list[xce]['isTesterPkt']:
                        ...
                        if response_atn != 0 and self.file_list[response_atn]['pktType'] == 'ATN':
                            check_atn = True
                            res.append([f"ATN response was observed for the XCE packet at index-{self.file_list[response_atn]['rowIndex']}", "Pass"])
                            break
                # print(check_atn)
                if check_atn:
                    ...
                    for dsr in range(response_atn, new_limit[1]):
                        if 'DSR' in self.file_list[dsr]['pktType'] and 'POLL' in self.file_list[dsr]['value'] and not self.file_list[dsr]['isTesterPkt']:
                            check_dsr = True
                            res.append([f"DSR-POLL packet was observed at index-{self.file_list[dsr]['rowIndex']}", "Pass"])
                            break
                if not check_atn:
                    res.append([f"ATN response was not observed for any XCE packet after the stable", "Inconclusive"])
                if not check_dsr:
                    res.append([f"DSR-POLL packet was not observed for the ATN packet", "Inconclusive"])
                ...
                for clk in range(dsr, new_limit[1]):
                    if 'Cloak' in self.file_list[clk]['pktType'] and self.file_list[clk]['isTesterPkt']:
                        ...
                        if Check['reason'] in reason:
                            ...
                            res.append([f"Cloak by Prx was initiated at index-{self.file_list[clk]['rowIndex']} sec with reason {reason}", "Pass"])  
                            ...
                            if self.file_list[cloak_respons_by_ptx]['pktType'] == 'Cloak' and not self.file_list[cloak_respons_by_ptx]['isTesterPkt']:   
                                ...
                                res.append([f"Cloak by PTx was observed at index-{self.file_list[cloak_respons_by_ptx]['rowIndex']}in response to Prx cloak", "Pass"])    
                            ...
                            if self.file_list[cloak_ack]['pktType'] == 'ACK' and self.file_list[cloak_ack]['isTesterPkt']:
                                ...
                                cloak_1 = cloak_ack
                                res.append([f"ACK response was observed for the PTx cloak packet at {self.file_list[cloak_ack]['rowIndex']}", "Pass"])         
                        break
                if not check_cloak_prx:
                    res.append([f"Cloak by Prx was not initiated after the DSR-POLL packet", "Inconclusive"])
                if not check_cloak_ptx:
                    res.append([f"Cloak by PTx was not observed in response to Prx cloak", "Inconclusive"])
                if not cloak_ack_check:
                    res.append([f"ACK response was not observed for the PTx cloak packet", "Inconclusive"])
                ...
                if check_cloak_prx and check_cloak_ptx and cloak_ack_check:
                    ...
                    if len(new_limit_1) == 2:
                        ...
                        if len(assertions_details) >= 3:
                            limit_4 = 0
                            res.append([f"{Check['cloak_cycle']}x cloak ping cycle was observed during the cloak phase, Cloak packet indexes - {check_clk_list}", "Pass"])                         
                            ...
                            for idx, (shrt, clk_2) in enumerate(zip(shortpings, assertions_details)):   
                                pings.append(shrt[0])
                                res.append([f"Received Short pings at indexes : {shrt[0]} for ASK cloak sequence:{idx+1}, Cloak index: {clk_2[1]}","Pass"])
                            ...
                            if len(pings) == len(assertions_details):
                                ...
                                for tact in pings:
                                    ...
                                    if Check['t_dactive'] == t_dactive:
                                        res.append([f"t_dactive measured between Cloak Detect Ping attach and detach is {t_dactive} ms which is equal to expected value {Check['t_dactive']} ms - measured index: [{tact}]", "Pass"])
                                    else: res.append([f"t_dactive measured between Cloak Detect Ping attach and detach is {t_dactive} ms which is not equal to expected value {Check['t_dactive']} ms - measured index: [{tact}]", "Fail"])
                                ...
                                for att in range(sig, self.Flow_limit[1]):
                                    if 'Cloak_Ping_Attach' in self.file_list[att]['pktType'] and self.file_list[att]['isTesterPkt']:
                                        ...
                                        res.append([f"Cloak_Ping_Attach assertion observed at index: {self.file_list[att]['rowIndex']}", "Pass"])
                                        break
                                if check_ping_attach:
                                    ...
                                    for ping_ph in range(limit_4, new_limit_1[1]):
                                        if 'Signal strength' in self.file_list[ping_ph]['pktType'] and 'Cloak' in self.file_list[ping_ph]['description']:
                                            res.append([f"Prx Doesn't send ASK Cloak packets", "Pass"])
                                            res.append([f"TPT entered into Ping phase at index-{self.file_list[ping_ph]['rowIndex']}", "Pass"])
                                            check_ping_phase = True
                                            break
                                    if not check_ping_phase:
                                        res.append([f"TPT did not enter into Ping phase after the N cloak cycles", "Fail"])
                                else:
                                    res.append([f"Cloak_Ping_Attach assertion not observed", "Inconclusive"])
                                
                            else: res.append([f"Expected Short pings not observed", "Inconclusive"])
                        else: res.append([f"{Check['cloak_cycle']}x cloak ping cycle was observed during the cloak phase", "Inconclusive"])

            else:
                res.append([f"Cloak Detect Ping value was 0 or not observed, expected non zero value", "Inconclusive"])
    ...
    def clk_Response(self,CTSCheck,Check,flows,flwID):
        ...
        if pt_pass and cl_pass:
                        ...
                        for dsr in range(response_atn, self.Flow_limit[1]):
                            if 'DSR' in self.file_list[dsr]['pktType'] and 'POLL' in self.file_list[dsr]['value'] and not self.file_list[dsr]['isTesterPkt']:
                                ...
                                if self.file_list[cloak_response]['pktType'] == 'Cloak' and self.file_list[cloak_response]['isTesterPkt']:
                                    ...
                                    if Check['reason'] in reason:
                                        res.append([f"Cloak with Reason:{reason} by TPT was initiated at index-{self.file_list[cloak_response]['rowIndex']} in response to DSR-POLL packet", "Pass"])
                                prx_cloak = self.GetPacketResponse3(cloak_response, [cloak_response, self.Flow_limit[1]], uut = True)
                                if self.file_list[prx_cloak]['pktType'] == 'Cloak' and not self.file_list[prx_cloak]['isTesterPkt']:
                                    prx_cloak_1 = True
                                    res.append([f"Cloak by Prx was observed at index-{self.file_list[prx_cloak]['rowIndex']} in response to TPT cloak", "Pass"])
                                ...
                                if self.file_list[prx_clk_response]['pktType'] == 'ACK' and self.file_list[prx_clk_response]['isTesterPkt']:
                                    ...
                                    res.append([f"ACK response was observed for the PRx cloak packet at index-{self.file_list[prx_clk_response]['rowIndex']}", "Pass"])
                                break
                        break
            if not check_atn:
                res.append([f"ATN response was not observed for any XCE packet", "Inconclusive"])
            if not check_dsr:
                res.append([f"DSR-POLL packet was not observed for the ATN packet", "Inconclusive"])
            if not ptx_cloak:
                res.append([f"Cloak by TPT was not initiated in response to DSR-POLL packet", "Inconclusive"])
            if not prx_cloak_1:
                res.append([f"Cloak by Prx was not observed in response to TPT cloak", "Inconclusive"])
                    ...
                    if len(check_clk_list) >= 3:
                        ...
                        for atn_1 in range(last_clk, new_limit_1[1]):
                            if 'Cloak' in self.file_list[atn_1]['pktType'] and not self.file_list[atn_1]['isTesterPkt']:
                                ...
                                if response_atn != 0 and self.file_list[response_atn]['pktType'] == 'ATN':
                                    res.append([f"ATN response was observed for the cloak packet at index-{self.file_list[response_atn]['rowIndex']}", "Pass"])
                                    ...
                                    for report in range(response_atn, new_limit_1[1]):
                                        if 'Report' in self.file_list[report]['pktType'] and not self.file_list[report]['isTesterPkt']:
                                            ...
                                            if 'Receiver Identification' in report_ID or 'PRx Identification' in report_ID:
                                                ...
                                                res.append([f"Report packet with PRx Identification found at index-{self.file_list[report]['rowIndex']}", "Pass"])
                                                ...
                                                if Check['response'] == 'NAK':
                                                    if self.file_list[nak_res]['pktType'] == 'NAK' and self.file_list[nak_res]['isTesterPkt']:
                                                        res.append([f"TPT responded with NAK for the report packet at {self.file_list[nak_res]['rowIndex']}", "Pass"])
                                                        check_nak = True
                                                        #check PRx doesn't send ASK Cloak
                                                    clk_aft = False
                                                    for check_clk in range(nak_res+1, new_limit_1[1]):
                                                        if self.file_list[check_clk]['pktType'] == 'Cloak':
                                                            res.append([f"PRx sends ASK Cloak after unexpected response", "Fail"])
                                                            clk_aft = True
                                                            break
                                                    if not clk_aft:
                                                        res.append([f"PRx doesn't send ASK Cloak after unexpected response", "Pass"])
                                                    chk_png = False
                                                    print(nak_res+1, self.Flow_limit[1])
                                                    id = nak_res+1
                                                    while id < self.Flow_limit[1]+30:
                                                        if 'Signal strength' in self.file_list[id]['pktType']:
                                                            res.append([f"TPT entered into Ping phase[{self.file_list[id]['pktType']}] at index-{self.file_list[id]['rowIndex']} after the unexpected response for the report packet", "Pass"])
                                                            chk_png = True
                                                    ...
                                                    if not chk_png:
                                                        res.append([f"TPT did not enter into Ping phase after the unexpected response for the report packet", "Fail"])                                                      
                                                    break
                                                if Check['response'] == 'ACK':
                                                    if self.file_list[nak_res]['pktType'] == 'ACK' and self.file_list[nak_res]['isTesterPkt']:
                                                        res.append([f"TPT responded with ACK for the report packet at index-{self.file_list[nak_res]['rowIndex']}", "Pass"])
                                                        ...
                                                        for get_re in range(nak_res, self.Flow_limit[1]):
                                                            if 'Get Request' in self.file_list[get_re]['pktType'] and not self.file_list[get_re]['isTesterPkt'] and 'PTx Extended Identification' in self.file_list[get_re]['value']:
                                                                res.append([f"GET[PTx XID] request was found at index-{self.file_list[get_re]['rowIndex']}", "Pass"])
                                                                ...
                                                                if self.file_list[response_xid]['pktType'] == EPTI and self.file_list[response_xid]['isTesterPkt']:
                                                                    res.append([f"TPT responded with XID packet for the request GET[PTx XID] at index-{self.file_list[response_xid]['rowIndex']}", "Pass"])
                                                                    check_response_xid = True
                        ...
                        if not check_cloak_with_atn:
                            res.append([f"ATN response was not observed for any cloak packet after the {Check['cloak_cycle']}x cloak ping cycles", "Inconclusive"])
                        if not check_report_with_prx_id:
                            res.append([f"Report packet with PRx Identification was not found after the ATN response for cloak packet", "Inconclusive"])
                        if not check_nak and Check['response'] == 'NAK':
                            res.append([f"TPT did not respond with NAK for the report packet with PRx Identification", "Inconclusive"])
                        if not check_get_ptx_xid and Check['response'] == 'ACK':
                            res.append([f"GET[PTx XID] request was not found after the ACK response for the report packet with PRx Identification", "Inconclusive"])
                        if not check_response_xid and Check['response'] == 'ACK':
                            res.append([f"TPT did not respond with XID packet for the request GET[PTx XID]", "Inconclusive"])
                        if check_cloak_with_atn and check_report_with_prx_id and check_nak and Check['reason'] == 'NAK':
                            ...
                            for ping_ph in range(nak_res, new_limit_1[1]):
                                if 'Signal strength' in self.file_list[ping_ph]['pktType'] and 'Cloak' in self.file_list[ping_ph]['description']:
                                    res.append([f"TPT entered into Ping phase at index-{self.file_list[ping_ph]['rowIndex']}, after the NAK response for the report packet", "Pass"])
                                    check_ping_phase = True                   
                            if not check_ping_phase:
                                res.append([f"TPT did not enter into Ping phase after the NAK response for the report packet", "Fail"])
                        if check_cloak_with_atn and check_report_with_prx_id and check_get_ptx_xid and check_response_xid and Check['response'] == 'ACK':
                            ...
                            for check_xid in range(self.Flow_limit[0], new_limit_1[1]):
                                ...
                                if EPTI in self.file_list[check_xid]['pktType'] and self.file_list[check_xid]['isTesterPkt']:           
                                    ...
                                    # else: res.append([f"TPT did not respond with XID packet for the request GET[PTx XID] before the cloak phase", "Fail"])
                                    break
                            ...
                            if value_before_cloak != value_after_cloak:
                                res.append([f"Device Identifier value in XID response packet is different before and after the cloak phase, value before cloak: {value_before_cloak}, value after cloak: {value_after_cloak}", "Pass"])
                                ...
                                for dig_ping in range(response_xid, len(self.file_list)):
                                    if 'Signal strength' in self.file_list[dig_ping]['pktType'] and not self.file_list[dig_ping]['isTesterPkt']:
                                        res.append([f"Digital Ping packet was observed at {self.file_list[dig_ping]['rowIndex']} sec after the XID response with mismatched identifier", "Pass"])
                                        check_digital_ping = True
                                        break
                                if not check_digital_ping:
                                    res.append([f"Digital Ping packet was not observed after the XID response with mismatched identifier", "Fail"])
                            else: res.append([f"Device Identifier value in XID response packet is same before and after the cloak phase, value before cloak: {value_before_cloak}, value after cloak: {value_after_cloak}", "Inconclusive"])
                    else: res.append([f"{Check['cloak_cycle']}x cloak ping cycle was observed during the cloak phase, Cloak packet indexes - {check_clk_list}", "Fail"])
    ...
    def EDS_check_1(self,CTSCheck,Check,flows,flwID):
        ...
        for eds in range(self.Flow_limit[0], self.Flow_limit[1]):
            if 'Enabled Data Streams' in self.file_list[eds]['pktType'] and not self.file_list[eds]['isTesterPkt']:
                eds_initiated = True
                res.append([f"Prx Requested Enabled Data Streams packet at {round(self.file_list[eds]['startTime'], 3)}sec", "Pass"])
                ...
                if self.file_list[response_eds]['pktType'] == 'Enabled Data Streams':
                    res.append([f"PTx responded with Enabled Data Streams packet for EDS packet request from Prx at {round(self.file_list[response_eds]['startTime'], 3)}sec", "Pass"])
        ...
        if pt_pass:
            ...
            print(len(sequence))
            if len(sequence) >= 1:
                res.append([f"{len(sequence)} Auth sequences were found & indexes are {sequence}", "Pass"])
                for seq in sequence:
                    check_res_open = False
                    check_res_close = False
                    check_res_open_1 = False
                    check_res_close_1 = False
                    res.append([f"Sequence {seq_count} -> index of {seq}", "Pass"])
                    ...
                    while id < seq[1]:
                        if ("SADC" in self.file_list[id]['pktType'] and "Open" in self.file_list[id]['value'] and not self.file_list[id]['isTesterPkt']):
                            ...
                            #     res.append([f"SDSR {self.file_list[id]['value']} response packet found at {round(self.file_list[id]['startTime'], 3)}sec", "Pass"])
                            ...
                            for res_1 in range(id, che+1):
                                if 'SDSR' in self.file_list[res_1]['pktType'] and 'ACK' in self.file_list[res_1]['value'] and self.file_list[res_1]['isTesterPkt']:
                                    check_res_open = True
                                    # print("Check",res_1)
                                    res.append([f"SDSR- ACK response was found for SADC-Open packet at index-{self.file_list[res_1]['rowIndex']}", "Pass"])
                                    id = res_1+1
                                    break
                            if not check_res_open:
                                res.append([f"SDSR response was not found for SADC-Open packet", "Inconclusive"])
                            ...
                            while uid < seq[1]:
                                if ("SADC" in self.file_list[uid]['pktType'] and "Close" in self.file_list[uid]['value'] and not self.file_list[uid]['isTesterPkt']):
                                    ...
                                    res.append([msg + 'SADT sequences', "Pass"])

                                    print("Even received:", even_count)
                                    print("Odd received:", odd_count)
                                    if temp_bytes == int(bytes_value):
                                        res.append([f"Transferred bytes {temp_bytes} observed from SADT packets.{odd_even}", "Pass"])
                                        res.append([
                                            ...
                                            if 'ATN' in self.file_list[cl]['pktType'] and self.file_list[cl]['isTesterPkt']:
                                                ...
                                                for res_2 in range(temp_limit[1], cl):
                                                    if 'SDSR' in self.file_list[res_2]['pktType'] and 'ACK' in self.file_list[res_2]['value'] and self.file_list[res_2]['isTesterPkt']:
                                                        ...
                                                        # print("Check",res_1)
                                                        res.append([f"SDSR- ACK response was found for SADC-Close packet at index-{self.file_list[res_2]['rowIndex']}", "Pass"])
                                                        break                        
                                                break
                                        if not check_res_close:
                                            res.append([f"SDSR response was not found for the SADC-close packet", "Inconclusive"])
                                    ...
                                    for temp in range(temp_id, seq[1] + 1):
                                        if 'SADC' in self.file_list[temp]['pktType'] and 'Open' in self.file_list[temp]['value'] and self.file_list[temp]['isTesterPkt'] and not self.file_list[temp][
                                            ...
                                            res.append([f"{self.file_list[temp]['pktType']}{self.file_list[temp]['value']} by tester packet found at "
                                                                                        f"index-{self.file_list[temp]['rowIndex']}", "Pass"])
                                            ...
                                            if len(temp_pkt_3) >= 2:
                                                    ...
                                                    if 'SADT' in self.file_list[op_1]['pktType'] and self.file_list[op_1]['isTesterPkt']:
                                                        ...
                                                        for res_3 in range(open_pkt, op_1+1):                       
                                                            if 'SDSR' in self.file_list[res_3]['pktType'] and not self.file_list[res_3]['isTesterPkt']:
                                                                ...
                                                                res.append([f"SDSR- ACK response was found for SADC-Open packet by Prx at index-{self.file_list[res_3]['rowIndex']}", "Pass"])
                                                                break
                                                        break
                                                if not check_res_open_1:
                                                    res.append([f"SDSR response was not found for SADC-Open packet by Prx", "Inconclusive"])
                                                temp += 1
                                                # print(temp, seq[1])
                                                    ...
                                                    if sadt_index is not None:
                                                        ...
                                                        for pkt, idx in check_DSR[sadt_index + 1:]:
                                                            ...
                                                            # If another SADT appears before DSR → FAIL
                                                                ...
                                                                res.append([f'Second SADT packet found at index {idx} before DSR [Index{temp_ids}]', "Fail"])
                                                                result_found = True
                                                                break
                                                        # If neither found
                                                        if not result_found:
                                                            res.append([f'DSR packet not found after first SADT sequence [Index{temp_ids}]', "Fail"])
                                                    else:
                                                        res.append([f'First SADT packet not found [Index{temp_ids}]', "Inconclusive"])
                                                respons = temp_ids[0]
                                                temp_bytes_1 = 0
                                                ...
                                                if temp_bytes_1 == int(bytes_value_1):
                                                    res.append([f"Transferred bytes {temp_bytes_1} observed from SADT packets.{odd_even_1}", "Pass"])
                                                    res.append([
                                                    ...
                                                    for cl_2 in range(temp_ids[1], temp_ids[1]+8):
                                                        if 'SDSR' in self.file_list[cl_2]['pktType'] and 'ACK' in self.file_list[cl_2]['value'] and not self.file_list[cl_2]['isTesterPkt']:
                                                            res.append([f"SDSR- ACK response was found for SADC-Close packet at index-{self.file_list[cl_2]['rowIndex']}", "Pass"])
                                                            check_res_close_1 = True
                                                            break
                                                    if not check_res_close_1:
                                                        res.append([f"SDSR response was not found for SADC-Close packet", "Inconclusive"])
                    ...
                    if not sadc_open_found:
                        res.append([f"SADC_open by PRx was not found", "Inconclusive"])
                    if not sadc_close_found:
                        res.append([f"SADC_close by PRx was not found", "Inconclusive"])
                    if not atn_found:
                        res.append([f"ATN response was not found", "Inconclusive"])
                    if not dsr_found:
                        res.append([f"DSR packet was not found", "Inconclusive"])
                    if not sadc_open_tester_found:
                        res.append([f"SADC_open by Tester was not found", "Inconclusive"])
                    if not sadc_close_tester_found:
                        res.append([f"SADC_close by Tester was not found", "Inconclusive"])
                # check tdts timings
                if Check['tdts'] is not None:
                    ...
                    for seq_name, packets in t_dts_data.items():
                        ...
                        for i in range(len(packets) - 1):
                            ...
                            if pair in occ:
                                occ[pair] += 1
                                status = "Pass" if tdts <= 5000 else "Fail"
                    ...
                    for pair in order:
                        for line, status in grouped_results[pair]:
                            if status == 'Pass':
                                res.append([line, "Pass"])
                            else:
                                res.append([line, "Fail"])  # print(line)
            elif len(sequence) == 0:
                sadc_1_check = False
                    ...
                    if check_sadc_open and check_sadc_close:
                        ...
                        for atn in range(limit, self.Flow_limit[1]+1):
                            if 'ATN' in self.file_list[atn]['pktType'] and self.file_list[atn]['isTesterPkt']:
                                atn_check = True
                                res.append([f"ATN resposne was observed at {round(self.file_list[atn]['startTime'], 3)}sec", "Pass"])
                                for dsr in range(atn, self.Flow_limit[1]):
                                    if 'DSR' in self.file_list[dsr]['pktType'] and not self.file_list[dsr]['isTesterPkt']:
                                        dsr_check = True
                                        res.append([f"DSR-POLL packet was observed at {round(self.file_list[dsr]['startTime'], 3)}sec", "Pass"])
                                        limit = dsr
                                        break
                                break
                        if not atn_check:
                            res.append([f"ATN resposne was not observed", "Inconclusive"])
                        if not dsr_check:
                            res.append([f"DSR resposne was not observed", "Inconclusive"])
                            ...
                            if len(inte_limit) == 2:
                                            ...
                                            if 'SADT' in self.file_list[ch]['pktType'] and self.file_list[ch]['isTesterPkt']:
                                                ...
                                                for response in range(in_index, lim):
                                                    if 'SDSR' in self.file_list[response]['pktType'] and 'ACK' in self.file_list[response]['value']:
                                                        res.append([f"SADT 1st packet with SDSR response was observed at index-{self.file_list[response]['rowIndex']}", "Pass"])
                                                        sadt_1_check = True
                                                        sdsr = response
                                                        break
                                                break
                                            ...
                                            if 'SADT' in self.file_list[ch_2]['pktType'] and self.file_list[ch_2]['isTesterPkt']:
                                                ...
                                                for response_1 in range(sdsr+1,ch_2-1):
                                                    if 'SDSR' in self.file_list[response_1]['pktType'] and 'ACK' in self.file_list[response_1]['value']:
                                                        res.append([f"2nd SADT packet was observed at index-{self.file_list[response_1]['rowIndex']}", "Inconclusive"])
                                                        sadt_2_check = False
                                                        break
                                                if not sadt_2_check: res.append([f"2nd SADT packet muted after 1st SADT sequence", "Pass"])
                                                break
                                    count += 1
                                ...
                                for dsr_retry in range(sdsr, lim+1):
                                    if 'DSR' in self.file_list[dsr_retry]['pktType'] and not self.file_list[dsr_retry]['isTesterPkt'] and '{POLL}' in self.file_list[dsr_retry]['value']:
                                        res.append([f"DSR/POLL retry packet was observed at {round(self.file_list[dsr_retry]['startTime'], 3)}sec", "Pass"])
                                        dsr_retry_check = True
                                        break
                                
                                if not sadt_1_check: res.append([f"1st SADT packet not observed with SDSR response", "Inconclusive"])           
                                if not dsr_retry_check: res.append([f"DSR/POLL retry packet was not observed after the 2nd SADT packet", "Inconclusive"])


                                    
                            else: res.append([f"Auth sequence by PTx not initiated", "Inconclusive"])

                                
                else: res.append([f"No Auth count sequence found, which is not expected, at least 1 sequence should initiate", "Fail"])


            else:
                res.append([f"No Auth count sequence found, which is not expected, at least 1 sequence should initiate", "Fail"])
            # print(t_dts_data)
        else:
            res.append([f"The packet sequence doesn't entered PT phase", "Inconclusive"])
        return res
                ...
                for seq in sequence:
                    ...
                    for ids in range(seq[0], seq[1]):
                        if 'ATN' in self.file_list[ids]['pktType'] and self.file_list[ids]['isTesterPkt']:
                            atn_check = True
                            chh = ids
                            res.append([f"ATN response packet found at index-{self.file_list[ids]['rowIndex']}", "Pass"])
                            break
                    if atn_check:
                        for idd in range(chh, seq[1]):
                            if 'DSR' in self.file_list[idd]['pktType'] and not self.file_list[idd]['isTesterPkt'] and '{POLL}' in self.file_list[idd]['value']:
                                dsr_check = True
                                res.append([f"DSR/POLL packet found at index-{self.file_list[idd]['rowIndex']}", "Pass"])
                                break
                    if not atn_check:
                        res.append([f"ATN response packet not found", "Fail"])
                    if not dsr_check:
                        res.append([f"DSR/POLL packet not found", "Fail"])
                    if atn_check and dsr_check:
                        ...
                        if seq_count == 1 and not ack_observed:
                            res.append([f"SDSR ACK response packet not found for the Auth sequence {seq_count} SADC packet [Index{temp_ids}]", "Fail"])
                        elif seq_count == 2 and not err_crc:
                            res.append([f"SDSR ERR_CRC response packet not found for the Auth sequence {seq_count} SADC packet [Index{temp_ids}]", "Fail"])
                        seq_count += 1
                if retry_sequence != 0:
                    ...
                    if sadc_open_check and sadc_close_check:
                        res.append([f"Retried Auth sequence after the ERR_CRC response found at the index [{retry_open, retry_close}]", "Pass"])
                    else:
                        res.append([f"Retried Auth sequence after the ERR_CRC response not found", "Inconclusive"])
        else:
            res.append([f"The packet sequence doesn't entered PT phase", "Inconclusive"])
        return res
    ...
    def EDS_check_2(self,CTSCheck,Check,flows,flwID):
        ...
        for eds in range(self.Flow_limit[0], self.Flow_limit[1]):
            if 'Enabled Data Streams' in self.file_list[eds]['pktType'] and not self.file_list[eds]['isTesterPkt']:
                eds_initiated = True
                res.append([f"Prx Requested Enabled Data Streams packet at index-{self.file_list[eds]['rowIndex']}", "Pass"])
                response_eds = self.GetPacketResponse3(eds, [eds,self.Flow_limit[1]])
                if self.file_list[response_eds]['pktType'] == 'Enabled Data Streams':
                    res.append([f"PTx responded with Enabled Data Streams packet for EDS packet request from Prx at index-{self.file_list[response_eds]['rowIndex']}", "Pass"])
        if not eds_initiated:
            pass  #if requires can edit later
        #check sequence entered the PT phase                    
        pt_pass = False
            ...
            if Check['check_SDSR'] is not None:
                for id in range(self.Flow_limit[0], self.Flow_limit[1]):
                    ...
                    if self.file_list[id]['pktType'] == 'SADC' and 'Open' in self.file_list[id]['value']:
                        sadc_check = True
                        res.append([f"SADC{self.file_list[id]['value']} packet by DUT found at index-{self.file_list[id]['rowIndex']}", "Pass"])
                        id += 1
                        if self.file_list[id]['pktType'] == 'SDSR' and Check['check_SDSR'] in self.file_list[id]['value']:
                            res.append([f"SDSR{self.file_list[id]['value']} packet by tester found at index-{self.file_list[id]['rowIndex']}", "Pass"])
                            sdsr_check = True
                            temp_id = id + 1
                            break
                ...
                if sadc_check and sadc_close and sdsr_check:
                    for check in range(new_limit[0], new_limit[1]):
                        if self.file_list[check]['pktType'] == 'SADC' and 'Open' in self.file_list[check]['value']:
                            ...
                            break
                if not sdsr_check:
                    res.append([f"SDSR {Check['check_SDSR']} packet by tester not found", "Fail"])
                if not sadc_check:
                    res.append([f"SADC Open packet by DUT not found", "Fail"])
                if not sadc_check_1:
                    res.append([f"retried SADC Open packet by DUT not found", "Fail"])
                if not sadc_close:
                    res.append([f"SADC close packet by DUT not found", "Fail"])
            else:
                temp_ids = []
                temp = 0
                for id in range(self.Flow_limit[0], self.Flow_limit[1]):
                    ...
                    for fsk in range(temp_ids[0], temp_ids[1]):
                        if 'SADT' in self.file_list[fsk]['pktType']:
                            ...
                            if self.file_list[response]['pktType'] == Check['response']:
                                res.append([f"NAK response observed for the SADT packet at index-{self.file_list[response]['rowIndex']}", "Pass"])
                                nak_check = True
                            elif self.file_list[response]['pktType'] != Check['response'] and nak_check:
                                res.append([
                                    f"{self.file_list[response]['pktType']} Resposne observed for the retried SADT packet at index-{self.file_list[response]['rowIndex']}", "Pass"])
                            else:
                                res.append([f"NAK response was not observed for the SADT packet", "Fail"])
                if not sadc_check:
                    res.append([f"SADC Open packet by DUT not found", "Fail"])
                if not sadc_close:
                    res.append([f"SADC close packet by DUT not found", "Fail"])
    ...
    def EDS_cloak(self,CTSCheck,Check,flows,flwID):
        ...
        if pt_pass:
            ...
            for id in range(self.Flow_limit[0], self.Flow_limit[1]):
                if 'SADC' in self.file_list[id]['pktType'] and 'Open' in self.file_list[id]['value'] and not self.file_list[id]['isTesterPkt']:
                    sadc_open_check = True
                    res.append([f"SADC Open packet by DUT found at index-{self.file_list[id]['rowIndex']}", "Pass"])
                    id += 1
                    for sadt in range(id, self.Flow_limit[1]):
                        ...
                        if 'SADT' in self.file_list[sadt]['pktType'] and not self.file_list[sadt]['isTesterPkt']:
                            res.append([f"SADT_1 packet by DUT found at index-{self.file_list[sadt]['rowIndex']}", "Pass"])
                            sadt_check = True
                            temp_id = sadt+1
                            break
                    break
            ...
            if sadc_open_check and sadt_check:
                for xce in range(temp_id, self.Flow_limit[1]):
                    if 'Extended Control Error' in self.file_list[xce]['pktType']:
                        xce_1_found = True
                        res.append([f"XCE first packet found at index-{self.file_list[xce]['rowIndex']}", "Pass"])
                        xce += 1
                        response = self.GetPacketResponse3(xce, [xce, self.Flow_limit[1]])
                        if self.file_list[response]['pktType'] == 'ATN':
                            atn_check = True
                            res.append([f"ATN response found for the first XCE packet at index-{self.file_list[response]['rowIndex']}", "Pass"])
                            temp_id = response + 1
                            break
                        break
            ...
            if xce_1_found and atn_check:
                for clk in range(temp_id, self.Flow_limit[1]):
                    if 'DSR' in self.file_list[clk]['pktType'] and '{POLL}' in self.file_list[clk]['value']:
                        dsr_check = True
                        res.append([f"DSR/POLL packet found at index-{self.file_list[clk]['rowIndex']}", "Pass"])
                        clk += 1
                        for cloak in range(clk, self.Flow_limit[1]):
                            if 'Cloak' in self.file_list[cloak]['pktType'] and self.file_list[cloak]['isTesterPkt']:
                                ptx_cloak = True
                                res.append([f"Cloak packet by Ptx found at index-{self.file_list[cloak]['rowIndex']}", "Pass"])
                                ...
                                if self.file_list[cloak]['header_Payload']['childelement'][0]['childelement'][1]['sDescription'] == 'Coex Mitigation':
                                    res.append([f"Cloak packet reason: Coex Mitigation found", "Pass"])
                                else:
                                    res.append([f"Cloak packet reason: Coex Mitigation not found", "Fail"])
                                cloak+=1
                                for cloak_prx in range(cloak, self.Flow_limit[1]):
                                    if 'Cloak' in self.file_list[cloak]['pktType'] and not self.file_list[cloak]['isTesterPkt']:
                                        prx_cloak = True
                                        temp_id = cloak_prx + 1
                                        res.append([f"Cloak packet by Prx found at {round(self.file_list[cloak_prx]['startTime'], 3)} sec", "Pass"])
                                        break
                                break
                        break
                ...
                if temp_ids:
                    ...
                    for clk in range(temp_ids[0], temp_ids[1]):  #check cloak cycles
                        if 'Cloak' in self.file_list[clk]['pktType'] and not self.file_list[clk]['isTesterPkt']:
                            ...
                            res.append([f"Cloak packet by tester initiated at index-{self.file_list[clk]['rowIndex']}" , "Pass"])
                    if len(cloak_cycle_count) >= 2:
                        cloak_cycle_count_check = True
                        res.append([f"Total {len(cloak_cycle_count)} cloak cycle count was observed.", "Pass"])
                    else: res.append([f"Expected cloak cycle count was not observed in the cloak sequence", "Fail"])
                else:
                    res.append([f"Cloak ping detect attach / Detach was not completed properly", "Inconclusive"])
            ...
            if cloak_cycle_count_check:
                ...
                if minutes >= 1.0:
                    ...
                    res.append([f"Streamer not waited for 1 minute of duration - Observed duration: {minutes}", "Inconclusive"])
                if not data_stream:
                    res.append([f"data stream was not started by PRX & limit: [{temp_ids[1], self.Flow_limit[1]}]", "Pass"])
                else:
                    #if data stream is there need to complete it
                    res.append([f"data stream was started by PRX at index-{self.file_list[data_stream_index]['rowIndex']}", "Pass"])
                    #logic for complete the sequence of auth sequence initiated

            #fallback logic if not got expected
            if not sadc_open_check:
                res.append([f"SADC Open packet by DUT not found", "Inconclusive"])
            if not sadt_check:
                res.append([f"SADT_1 packet by DUT not found", "Fail"])
            if not xce_1_found:
                res.append([f"XCE packet by DUT not found", "Fail"])
            if not atn_check:
                res.append([f"ATN response not found", "Fail"])
            if not dsr_check:
                res.append([f"DSR/POLL packet not found", "Fail"])
            if not ptx_cloak:
                res.append([f"Cloak by Ptx was not initiated", "Fail"])
            if not prx_cloak:
                res.append([f"Cloak by Prx was not initiated", "Fail"])
        else: res.append([f"The packet sequence doesn't entered PT phase", "Inconclusive"])
        return res
    ...
    def EDS_stream(self,CTSCheck,Check,flows,flwID):
        ...
        if pt_pass:
                ...
                for sadt in range(temp_id, temp_id_1):
                    if 'SADT' in self.file_list[sadt]['pktType']:
                        ...
                        if self.file_list[response]['pktType'] == 'ACK' or self.file_list[response]['pktType'] == 'NAK':
                            SADT_response = True
                if not SADT_check:
                    res.append([f"SADT packet was not found", "Inconclusive"])
                if not SADT_response:
                    res.append([f"TPT not responds to all SADT packets", "Fail"])
            ...
            if SADC_open_check and SDSR_open_check and SADC_close_check and SDSR_close_check and SADT_check and SADT_response:
                res.append([f"TPT responds to all Authentication request from PRx packets", "Pass"])
            else: res.append([f"TPT not responds to all Authentication request from PRx packets", "Fail"])
            ...
            if SADT_check and SADT_response:
                for atn in range(temp_id_1+1 , self.Flow_limit[1]):
                    if 'ATN' in self.file_list[atn]['pktType']:
                        atn_check = True
                        res.append([f"ATN response was found at index-{self.file_list[atn]['rowIndex']}", "Pass"])
                        for dsr in range(temp_id_1, self.Flow_limit[1]):
                            if 'DSR' in self.file_list[dsr]['pktType'] and '{POLL}' in self.file_list[dsr]['value']:
                                dsr_check = True
                                res.append([f"DSR/POLL packet was found at index-{self.file_list[dsr]['rowIndex']}", "Pass"])
                                temp_id = dsr
                                break
                        break

                if not atn_check:
                    res.append([f"ATN response was not initiated for the XCE packet", "Fail"])
                if not dsr_check:
                    res.append([f"DSR/POLL packet was not initiated", "Fail"])
            ...
            if atn_check and dsr_check and Check['timing_check_1min'] is None and Check['checking_even_data_stream'] is not None:
                ...
                for open_1 in range(dsr+1, self.Flow_limit[1]):
                    ...
                    if 'SADC' in self.file_list[open_1]['pktType'] and 'Open' in self.file_list[open_1]['value'] and self.file_list[open_1]['isTesterPkt']:
                        ...
                        else: res.append([f"SDSR-ACK response not observed for the SADC-Open packet", "Fail"])
                        #sadc_close              
                        for close_1 in range(response_1, self.Flow_limit[1]):
                            if 'SADC' in self.file_list[close_1]['pktType'] and 'Close' in self.file_list[close_1]['value'] and self.file_list[close_1]['isTesterPkt']:
                                ...
                                if self.file_list[response_2]['pktType'] == 'SDSR' and 'ACK' in self.file_list[response_2]['value']:
                                    ...
                                    sdsr_close_check = False
                                    index_sdsr = response_2
                                else: res.append([f"SDSR-ACK response not observed for the SADC-Close packet", "Fail"])
                                break
                        break
                if not SADC_open_check_1:
                    res.append([f"SADC-Open by Prx was not found", "Inconclusive"])
                if not SADC_close_check_1:
                    res.append([f"SADC-Close by Prx was not found", "Inconclusive"])
                ...
                if SADC_open_check_1 and SADC_close_check_1:
                    ...
                    if even_pattern_found:
                        res.append([f"Repeating SADT/even packet 3x on the same stream was found. indexes-[{found_index, found_index+2, found_index+4}]", "Pass"])
                        #check response for SADC_Close
                        if sdsr_close_check:
                            res.append([f"SDSR-ACK response was observed for SADC-Close packet at index-{self.file_list[index_sdsr]['rowIndex']}", "Pass"])
                        elif not sdsr_close_check:
                            res.append([f"SDSR-ACK response was not observed for SADC-Close packet, Observed packet: {self.file_list[index_sdsr]['pktType']}", "Fail"])
                    else: res.append([f"Repeating SADT/even packet 3x on the same stream was not found.", "Fail"])
            ...
            if atn_check and dsr_check and Check['timing_check_1min'] is None and Check['checking_even_data_stream'] is None:
                ...
                for rep in range(new_limit_2[0], self.Flow_limit[1]):                              
                    if 'SADC' in self.file_list[rep]['pktType'] and chk_1 in self.file_list[rep]['value'] and self.file_list[rep]['isTesterPkt']:
                        count += 1
                        if count == 1:
                            res.append([f"SADC-{chk_1} packet was found at index-{self.file_list[rep]['rowIndex']}", "Pass"])
                        elif count == 2: res.append([f"Repeat SADC-{chk_1} packet was found at index-{self.file_list[rep]['rowIndex']}", "Pass"])
                        else: pass
                        #check response
                        ...
                        if self.file_list[response_1]['pktType'] == 'SDSR' and 'ACK' in self.file_list[response_1]['value']:
                            if count == 1:
                                res.append([f"SDSR_ACK response was found for the SADC-{chk_1} packet at index-{self.file_list[response_1]['rowIndex']}", "Pass"])
                            elif count == 2:
                                res.append([f"Repeat sequence- SDSR_ACK response was found for the SADC-{chk_1} packet at index-{self.file_list[response_1]['rowIndex']}", "Pass"])
                        else: 
                            if count == 1:
                                res.append([f"SDSR_ACK response was not found for the SADC-{chk_1} packet", "Fail"])
                            elif count == 2:
                                res.append([f"Repeated sequence - SDSR_ACK response was not found for the SADC-{chk_1} packet", "Fail"])
                if count == 0:
                    res.append([f"SADC-{chk_1} packet was not found in the packet sequence", "Inconclusive"])
                    ...
                    if 'SADC' in self.file_list[check]['pktType'] and chk in self.file_list[check]['value'] and not self.file_list[check]['isTesterPkt']:
                        ...
                        if 'Qi Authentication' in value and Check['check_epp']:
                            ...
                            if not crc_proper:  #Will provide in loop result to get the non-proper index for user
                                res.append([f"checked CRC was not proper from SADC close packet at {crc_indexes}","Fail"])
                            crc_proper = False
                # print(epp_list)
                # print(crc_list)
                if Check['check_epp']:
                    if all(epp_list):
                        res.append([f"QI EPP are not Supported [Index - {temp_id, self.Flow_limit[1]}", "Pass"])
                    else: res.append([f"QI EPP are Supported [Index - {temp_id, self.Flow_limit[1]}", "Fail"])
                elif Check['check_CRC']:
                    if all(crc_list):
                        res.append([f"All measured CRC was proper by sending SDSR-ACK packet","Pass"])
                    else: res.append([f"All measured CRC was not proper by not sending SDSR-ACK packet, [Index - {temp_id, self.Flow_limit[1]}","Fail"])
            ...
            if not SADC_open_check:
                res.append([f"SADC_Open Packet by PRx was not found", "Inconclusive"])
            if not SDSR_open_check:
                res.append([f"SDSR response for SADC open Packet was not found", "Fail"])
            if not SADC_close_check:
                res.append([f"SADC_close Packet by PRx was not found", "Inconclusive"])
            if not SDSR_close_check:
                res.append([f"SDSR response for SADC close Packet was not found", "Fail"])
    ...
    def EDS_crc_handling(self,CTSCheck,Check,flows,flwID):
        ...
        if pt_pass:
                    ...
                    if SADC_open_check and SDSR_open_check and SDSR_close_check and SADC_close_check:
                        ...
                        for sadt in range(temp_id, temp_id_1+1):
                            if 'SADT' in self.file_list[sadt]['pktType']:
                                ...
                                else: res.append([f"Expected response was not observed for the SADT packet [index-{response}]", "Fail"])                                                      
                    break
            ...
            if SADC_open_check and SDSR_open_check and SADC_close_check and SDSR_close_check and SADT_response and all(sadt_responses):
                res.append([f"Prx Auth sequence observed with SDSR-ERR_CRC response at index- [{err_crc}] for the SADC close packet", "Pass"])
                ...
                res.append([f"All SADT responses were not proper, or the expected responses were not received.", "Fail"])
            else: res.append([f"Prx Auth sequence with SDSR-ERR_CRC response for the SADC close packet was not observed", "Fail"])
            ...
            if not SADC_close_check:
                res.append([f"SADC_close Packet by PRx was not found", "Inconclusive"])
            if not SDSR_close_check:
                res.append([f"Expected response of SDSR response for SADC close Packet was not found", "Fail"])
            ...
            if SDSR_close_check:
                ...
                if retry_auth_check_open and retry_auth_check_close:
                    res.append([f"Retry auth sequence was observed with SADC open at index {temp_id_2} and SADC close at index {temp_id_1}", "Pass"])
                else: res.append([f"Retry auth sequence was not observed properly", "Fail"])
                ...
                if retry_auth_check_open and retry_auth_check_close and retry_auth_sdsr_open and retry_auth_sdsr_close and all(sadt_responses_1):
                    res.append([f"Retry auth sequence was observed with proper response for the SADT packets", "Pass"])
                    ...
                    res.append([f"Retry auth sequence was observed but SADT responses were not proper, or the expected responses were not received.", "Fail"])
                else: res.append([f"Retry auth sequence with proper response was not observed", "Fail"])
